...
class MAD4B_SCP_Identity_Context {
	...
	public static function current() {
		$subject = 'wrong' === self::$mode ? 'wrong-ai-agent' : 'configured-ai-agent';
		return array(
			'authenticated' => true,
			'auth_method' => 'oauth2_bearer',
			'subject_fingerprint' => hash( 'sha256', $subject ),
			'issuer_fingerprint' => hash( 'sha256', 'https://issuer.example.test' ),
			'client_fingerprint' => hash( 'sha256', 'chatgpt-client' ),
			'session_fingerprint' => hash( 'sha256', 'session-' . self::$mode ),
		);
	}
}

class MAD4B_SCP_Agent_Registry {
	public static $grant_enabled = false;
	public static $configured_public_id = '22222222-2222-4222-8222-222222222222';
	public static $wrong_public_id = '33333333-3333-4333-8333-333333333333';

	private static function configured_agent() {
		return array(
			'id' => 7,
			'public_id' => self::$configured_public_id,
			'slug' => 'context-review-ai',
			'label' => 'Context Review AI',
			'status' => 'enabled',
			'environment' => 'staging',
		);
	}
	...
	public static function get_agent_by_public_id( $public_id ) {
		return self::$configured_public_id === (string) $public_id ? self::configured_agent() : null;
	}

	public static function resolve_agent( $identity ) {
		if ( ! is_array( $identity ) ) return new WP_Error( 'fixture_agent_identity_invalid', 'Identity fixture is invalid.' );
		if ( hash( 'sha256', 'wrong-ai-agent' ) === ( isset( $identity['subject_fingerprint'] ) ? (string) $identity['subject_fingerprint'] : '' ) ) {
			return array(
				'id' => 8,
				'public_id' => self::$wrong_public_id,
				'slug' => 'wrong-ai-agent',
				'label' => 'Wrong AI Agent',
				'status' => 'enabled',
				'environment' => 'staging',
			);
		}
	...
	public static function exact_grant( $agent_id, $server_id, $ability_name, $provider ) {
		if ( ! self::$grant_enabled ) return null;
		if ( 7 !== (int) $agent_id || 'mad4b-write' !== (string) $server_id || 'mad4b/context-ai-review' !== (string) $ability_name || 'core' !== (string) $provider ) return null;
		return array(
			'id' => 71,
			'agent_id' => 7,
			'server_id' => 'mad4b-write',
			'ability_name' => 'mad4b/context-ai-review',
			'provider' => 'core',
			'environment' => 'staging',
			'effect' => 'allow',
		);
...
// --- rc.54 delegated AI Agent approval mode ---

function mad4b_ai_review_input( $asset_id, $decision = 'approve', $note = '' ) {
	$asset = MAD4B_SCP_Context_Authority::asset( $asset_id );
	mad4b_review_assert( ! empty( $asset ), 'AI review input requires a live Context asset.', $asset_id );
	return array(
		'asset_id' => $asset_id,
		'decision' => $decision,
		'review_note' => $note,
...
$initial_policy = MAD4B_SCP_Context_Authority::review_policy();
mad4b_review_assert( 'human_only' === $initial_policy['mode'], 'AI review must fail closed to human_only by default.', $initial_policy );
mad4b_review_assert( empty( MAD4B_SCP_Context_Authority::ai_review_catalog_eligible() ), 'AI review catalog eligibility must be false before explicit delegation.' );

$policy_revision_before = MAD4B_SCP_Context_Authority::registry_revision();
$unconfirmed_policy = MAD4B_SCP_Context_Authority::set_review_policy(
	'human_and_ai',
	MAD4B_SCP_Agent_Registry::$configured_public_id,
	false
);
mad4b_review_assert( is_wp_error( $unconfirmed_policy ), 'AI review delegation must require explicit administrator confirmation.', $unconfirmed_policy );
mad4b_review_assert( 'mad4b_context_ai_review_confirmation_required' === $unconfirmed_policy->get_error_code(), 'Unconfirmed AI review delegation must expose the exact confirmation error.', $unconfirmed_policy->get_error_code() );
mad4b_review_assert( $policy_revision_before === MAD4B_SCP_Context_Authority::registry_revision(), 'Rejected delegation must not advance registry revision.' );

$wrong_agent_policy = MAD4B_SCP_Context_Authority::set_review_policy(
	'human_and_ai',
	MAD4B_SCP_Agent_Registry::$wrong_public_id,
	true
);
mad4b_review_assert( is_wp_error( $wrong_agent_policy ), 'AI delegation must reject an unconfigured/unregistered agent.', $wrong_agent_policy );
mad4b_review_assert( 'mad4b_context_ai_review_agent_ineligible' === $wrong_agent_policy->get_error_code(), 'Wrong AI Agent policy must fail with exact eligibility error.', $wrong_agent_policy->get_error_code() );

$before_policy_asset = MAD4B_SCP_Context_Authority::asset( $asset_id );
$policy_enabled = MAD4B_SCP_Context_Authority::set_review_policy(
	'human_and_ai',
	MAD4B_SCP_Agent_Registry::$configured_public_id,
	true
);
mad4b_review_assert( ! is_wp_error( $policy_enabled ), 'Administrator must be able to enable Human + AI review with an exact enabled Staging agent.', $policy_enabled );
mad4b_review_assert( 'human_and_ai' === $policy_enabled['mode'], 'Enabled review policy must persist human_and_ai mode.', $policy_enabled );
mad4b_review_assert( MAD4B_SCP_Agent_Registry::$configured_public_id === $policy_enabled['ai_agent_public_id'], 'Review policy must bind the exact selected AI Agent.', $policy_enabled );
mad4b_review_assert( $before_policy_asset === MAD4B_SCP_Context_Authority::asset( $asset_id ), 'Enabling AI approval mode must not review or mutate any Context asset.' );
mad4b_review_assert( ! empty( MAD4B_SCP_Context_Authority::ai_review_catalog_eligible() ), 'Explicit Human + AI policy must make the dedicated write candidate catalog-eligible.' );

$policy_status_without_grant = MAD4B_SCP_Context_Authority::ai_review_policy_status();
mad4b_review_assert( empty( $policy_status_without_grant['ready'] ), 'AI review policy must remain blocked until the exact grant exists.', $policy_status_without_grant );
mad4b_review_assert( in_array( 'ai_review_exact_grant_missing', $policy_status_without_grant['blockers'], true ), 'Missing exact AI review grant must remain visible.', $policy_status_without_grant );
mad4b_review_assert( empty( $policy_status_without_grant['grant_reconciliation_automatic'] ), 'Enabling AI review must never reconcile grants automatically.', $policy_status_without_grant );

MAD4B_SCP_Identity_Context::$mode = 'configured';
$no_grant_revision = MAD4B_SCP_Context_Authority::registry_revision();
$no_grant_review = MAD4B_SCP_Context_Authority::review_asset_by_agent( mad4b_ai_review_input( $asset_id ) );
mad4b_review_assert( is_wp_error( $no_grant_review ), 'AI review must fail closed without the exact Staging grant.', $no_grant_review );
mad4b_review_assert( 'mad4b_context_ai_review_exact_grant_missing' === $no_grant_review->get_error_code(), 'Missing AI grant must expose the exact grant error.', $no_grant_review->get_error_code() );
mad4b_review_assert( $no_grant_revision === MAD4B_SCP_Context_Authority::registry_revision(), 'Missing-grant AI review must not mutate registry state.' );

MAD4B_SCP_Agent_Registry::$grant_enabled = true;
...
mad4b_review_assert( is_wp_error( $wrong_identity_review ), 'Authenticated but non-delegated agent must be denied.', $wrong_identity_review );
mad4b_review_assert( 'mad4b_context_ai_review_agent_mismatch' === $wrong_identity_review->get_error_code(), 'Wrong authenticated agent must expose exact agent-mismatch error.', $wrong_identity_review->get_error_code() );
mad4b_review_assert( $wrong_identity_revision === MAD4B_SCP_Context_Authority::registry_revision(), 'Wrong-agent review must not mutate registry state.' );

MAD4B_SCP_Identity_Context::$mode = 'configured';
$forbidden_input = mad4b_ai_review_input( $asset_id );
$forbidden_input['category'] = 'brand_strategy';
$forbidden_review = MAD4B_SCP_Context_Authority::review_asset_by_agent( $forbidden_input );
mad4b_review_assert( is_wp_error( $forbidden_review ), 'AI review must reject caller-supplied governance metadata.', $forbidden_review );
mad4b_review_assert( 'mad4b_context_ai_review_governance_mutation_forbidden' === $forbidden_review->get_error_code(), 'AI governance mutation attempt must expose exact denial.', $forbidden_review->get_error_code() );

$stale_input = mad4b_ai_review_input( $asset_id );
$stale_input['expected_content_hash'] = str_repeat( 'f', 64 );
$stale_revision = MAD4B_SCP_Context_Authority::registry_revision();
$stale_review = MAD4B_SCP_Context_Authority::review_asset_by_agent( $stale_input );
mad4b_review_assert( is_wp_error( $stale_review ), 'AI review must fail closed on stale content evidence.', $stale_review );
mad4b_review_assert( 'mad4b_context_review_content_drift' === $stale_review->get_error_code(), 'AI stale-content denial must use the common exact-review drift contract.', $stale_review->get_error_code() );
mad4b_review_assert( $stale_revision === MAD4B_SCP_Context_Authority::registry_revision(), 'Stale AI review must not advance registry revision.' );

$before_ai_review = MAD4B_SCP_Context_Authority::asset( $asset_id );
$before_ai_governance = array(
	'category' => $before_ai_review['category'],
	'authority_class' => $before_ai_review['authority_class'],
	'required' => ! empty( $before_ai_review['required'] ),
	'classification_source' => isset( $before_ai_review['classification_source'] ) ? $before_ai_review['classification_source'] : '',
	'quality_score' => isset( $before_ai_review['quality_score'] ) ? $before_ai_review['quality_score'] : null,
	'quality' => isset( $before_ai_review['quality'] ) ? $before_ai_review['quality'] : array(),
);

$ai_approval = MAD4B_SCP_Context_Authority::review_asset_by_agent( mad4b_ai_review_input( $asset_id, 'approve' ) );
mad4b_review_assert( ! is_wp_error( $ai_approval ), 'Exact configured AI Agent must be able to approve current governed content.', $ai_approval );
mad4b_review_assert( 'approved' === $ai_approval['review_status'] && 'approve' === $ai_approval['review_decision'], 'AI approval must persist explicit approved decision state.', $ai_approval );
mad4b_review_assert( 'ai_agent' === $ai_approval['review_actor_type'], 'AI approval must persist actor_type=ai_agent.', $ai_approval );
...
mad4b_review_assert( $before_ai_governance['classification_source'] === ( isset( $ai_approval['classification_source'] ) ? $ai_approval['classification_source'] : '' ), 'AI review must not rewrite classification provenance as human.', $ai_approval );
mad4b_review_assert( $before_ai_governance['quality_score'] === ( isset( $ai_approval['quality_score'] ) ? $ai_approval['quality_score'] : null ), 'AI review must preserve quality score.', $ai_approval );
mad4b_review_assert( $before_ai_governance['quality'] === ( isset( $ai_approval['quality'] ) ? $ai_approval['quality'] : array() ), 'AI review must preserve quality policy/evidence byte-for-byte at array level.', $ai_approval );

$ai_events = array_values( array_filter( $GLOBALS['mad4b_context_audit'], static function ( $row ) { return 'mad4b/context-asset-ai-review' === $row['event']; } ) );
mad4b_review_assert( 1 === count( $ai_events ), 'Successful AI review must emit one dedicated audit event.', $ai_events );
$ai_event = $ai_events[0];
mad4b_review_assert( MAD4B_SCP_Context_Authority::AI_REVIEW_CONTRACT === $ai_event['data']['contract'], 'AI review audit must use the dedicated AI review contract.', $ai_event );
mad4b_review_assert( 'ai_agent' === $ai_event['data']['actor_type'], 'AI review audit must identify actor_type=ai_agent.', $ai_event );
mad4b_review_assert( MAD4B_SCP_Agent_Registry::$configured_public_id === $ai_event['data']['agent_public_id'], 'AI review audit must bind exact agent public ID.', $ai_event );
mad4b_review_assert( 64 === strlen( (string) $ai_event['data']['subject_fingerprint'] ) && 64 === strlen( (string) $ai_event['data']['issuer_fingerprint'] ), 'AI review audit must retain only bounded identity fingerprints.', $ai_event );
mad4b_review_assert( empty( $ai_event['data']['required_scope_changed'] ), 'AI review must never change site-wide Required Context scope.', $ai_event );

$human_override = MAD4B_SCP_Context_Authority::review_asset(
	$asset_id,
	mad4b_review_exact_input(
		$asset_id,
		array(
			'category' => $ai_approval['category'],
			'authority_class' => $ai_approval['authority_class'],
			'required' => ! empty( $ai_approval['required'] ),
			'quality_mode' => 'automatic',
			'quality_score' => '',
			'decision' => 'approve',
		)
...
mad4b_review_assert( ! is_wp_error( $human_override ), 'Human reviewer must remain able to override/reaffirm an AI decision.', $human_override );
mad4b_review_assert( 'wp_admin' === $human_override['review_actor_type'], 'Human override must restore actor_type=wp_admin.', $human_override );
mad4b_review_assert( '' === $human_override['review_agent_public_id'], 'Human override must clear AI agent attribution from current review state.', $human_override );
mad4b_review_assert( 42 === (int) $human_override['reviewed_by'], 'Human override must retain WordPress reviewer attribution.', $human_override );

$task_ai_revision = MAD4B_SCP_Context_Authority::registry_revision();
$task_ai_review = MAD4B_SCP_Context_Authority::review_asset_by_agent( mad4b_ai_review_input( $task_asset_id, 'approve' ) );
mad4b_review_assert( is_wp_error( $task_ai_review ), 'Task-local Context must remain ineligible for AI Agent approval.', $task_ai_review );
mad4b_review_assert( 'mad4b_context_review_source_mode_forbidden' === $task_ai_review->get_error_code(), 'Task-local AI review must fail with the shared governed-source boundary.', $task_ai_review->get_error_code() );
mad4b_review_assert( $task_ai_revision === MAD4B_SCP_Context_Authority::registry_revision(), 'Denied task-local AI review must not mutate registry state.' );

MAD4B_SCP_Site_Profile::$environment = 'production';
$production_review = MAD4B_SCP_Context_Authority::review_asset_by_agent( mad4b_ai_review_input( $asset_id ) );
mad4b_review_assert( is_wp_error( $production_review ), 'AI Agent approval must remain fail-closed in Production.', $production_review );
mad4b_review_assert( 'mad4b_context_ai_review_staging_only' === $production_review->get_error_code(), 'Production AI review must expose exact Staging-only denial.', $production_review->get_error_code() );
mad4b_review_assert( empty( MAD4B_SCP_Context_Authority::ai_review_catalog_eligible() ), 'Production must never expose AI review as runtime catalog-eligible.' );
MAD4B_SCP_Site_Profile::$environment = 'staging';

$human_only = MAD4B_SCP_Context_Authority::set_review_policy( 'human_only', '', false );
mad4b_review_assert( ! is_wp_error( $human_only ), 'Administrator must be able to disable AI review without extra confirmation.', $human_only );
mad4b_review_assert( 'human_only' === $human_only['mode'], 'Disabling AI review must restore human_only mode.', $human_only );
mad4b_review_assert( empty( MAD4B_SCP_Context_Authority::ai_review_catalog_eligible() ), 'Disabled AI review must leave the AI write candidate runtime-ineligible.' );
$final_asset = MAD4B_SCP_Context_Authority::asset( $asset_id );
mad4b_review_assert( 'approved' === $final_asset['review_status'], 'Disabling AI mode must not erase the current reviewed state.', $final_asset );

$policy_events = array_values( array_filter( $GLOBALS['mad4b_context_audit'], static function ( $row ) { return 'mad4b/context-review-policy-save' === $row['event']; } ) );
mad4b_review_assert( 2 === count( $policy_events ), 'AI mode enable and disable must each append one policy audit event.', $policy_events );
mad4b_review_assert( empty( $policy_events[0]['data']['grant_reconciliation_called'] ) && empty( $policy_events[1]['data']['grant_reconciliation_called'] ), 'Review policy changes must never reconcile grants automatically.', $policy_events );

echo "mad4b.site-control-plane.context-ai-agent-review.runtime.v1: PASS\n";
