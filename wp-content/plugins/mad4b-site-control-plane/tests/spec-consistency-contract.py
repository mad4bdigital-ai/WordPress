...
# Normative documentation must move with the physical schema. Schema v8 preserves
...
require(data_model, 'Schema version: `8`', 'data-model-schema-v8')
for table in (
...
for marker in (
    ...
    'clone', 'Site Profile drift', 'current expected version is `8`',
): require(data_model, marker, 'data-model-approval-v8')
for stale in (
    'Schema version: `5`',
    'Schema version: `6`',
...
require(impl['schema'], 'const VERSION = 8;', 'implementation-schema-v8')
...
): require(impl['schema'], marker, 'implementation-schema-v8-approval-guard')
for marker in (
