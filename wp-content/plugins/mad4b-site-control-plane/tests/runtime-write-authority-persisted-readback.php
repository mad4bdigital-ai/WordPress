...
if ( empty( $status['ready'] ) || 'ready' !== ( isset( $status['state'] ) ? (string) $status['state'] : '' ) ) {
	$fail( 'Persisted authority was not restored on the independent request: ' . wp_json_encode( $status ) );
}
...
$grant_snapshot = static function () use ( $agent ) {
	...
	usort( $canonical, static function ( $a, $b ) {
		return strcmp( $a['ability_name'] . "\0" . $a['provider'] . "\0" . $a['id'], $b['ability_name'] . "\0" . $b['provider'] . "\0" . $b['id'] );
	} );
	return array(
		'rows' => $canonical,
		'hash' => hash( 'sha256', wp_json_encode( $canonical, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE ) ),
	);
};
...
if ( ! empty( $live['wildcard_grants'] ) ) $fail( 'Wildcard grants appeared on the independent request.' );
if ( ! empty( $live['grant_blockers'] ) ) $fail( 'Grant blockers appeared on the independent request.' );

$cert = MAD4B_SCP_Live_Truth::current_write_certification();
if ( empty( $cert['ready'] ) ) $fail( 'Fresh write certification is not ready on the independent request: ' . wp_json_encode( $cert['blockers'] ?? array() ) );

MAD4B_SCP_Connection_Status::status();
MAD4B_SCP_Skill_Abilities::skills_export_status();

$after = $grant_snapshot();
if ( ! hash_equals( $before['hash'], $after['hash'] ) || $before['rows'] !== $after['rows'] ) {
	$fail( 'Independent read/status request mutated exact mad4b-write grants.' );
}
