...
foreach ( array( 'openai-chatgpt', 'anthropic-claude', 'google-gemini', 'manus', 'generic-mcp', 'future-agent' ) as $id ) {
	if ( ! in_array( $id, $manifest_ids, true ) ) mad4b_client_compat_fail( 'Expected dynamic compatibility profile missing from manifest.', $id );
}
...
fwrite( STDOUT, 'mad4b.site-control-plane.runtime-mcp-client-compatibility.v2: PASS' . PHP_EOL );
