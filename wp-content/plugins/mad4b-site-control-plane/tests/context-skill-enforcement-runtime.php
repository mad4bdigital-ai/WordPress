...
final class MAD4B_SCP_Skill_Registry {
	...
	public static function get_skill( $level, $target, $name ) {
		return array(
			...
			'context_policy' => array( 'preset' => 'brand_core', 'brand_context_required' => true ),
		);
	}
}
final class MAD4B_SCP_Context_Preflight {
	...
	public static function preflight_entry( array $skill, $task_scope = '' ) {
		return array(
			...
			'receipt' => array( 'contract' => 'mad4b.content-context-receipt.v1', 'receipt_sha256' => str_repeat( 'b', 64 ), 'ready' => self::$ready ),
		);
	}
}
...
$blocked = MAD4B_SCP_Skill_Abilities::skill_get( array( 'level' => 'site', 'name' => 'blog-writer', 'task_scope' => 'campaign-x' ) );
...
$ready = MAD4B_SCP_Skill_Abilities::skill_get( array( 'level' => 'site', 'name' => 'blog-writer', 'task_scope' => 'campaign-x' ) );
mad4b_skill_context_assert( is_array( $ready ) && 'mad4b.skill-get.v2' === $ready['contract'], 'Ready Skill exposure must use Context-bound contract.', $ready );
