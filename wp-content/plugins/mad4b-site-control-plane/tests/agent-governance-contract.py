...
require(authz, 'MAD4B_SCP_Approval_Tickets::authorize_exact', 'approval-readonly-preflight')
...
require(budgets, 'CLEANUP_LIMIT = 200', 'budget-bounded-cleanup')
...
require(budgets, 'public static function transaction_is_open', 'budget-owned-transaction-state')
require(budgets, 'MAD4B_SCP_Audit::transaction_committed()', 'audit-dispatch-after-budget-commit')
require(budgets, 'MAD4B_SCP_Audit::transaction_rolled_back()', 'audit-drop-after-budget-rollback')
...
require(audit, "$t['audit_events']", 'audit-events-table-use')
require(audit, "$t['audit_heads']", 'audit-heads-table-use')
require(audit, 'public static function ensure_head_initialized', 'audit-head-preinitialization')
require(plugin, 'MAD4B_SCP_Audit::ensure_head_initialized()', 'audit-head-boot-preinitialization')
require(audit, 'SELECT * FROM {$t[\'audit_heads\']} WHERE chain_name = %s LIMIT 1 FOR UPDATE', 'audit-head-lock')
...
require(audit, 'MAD4B_SCP_Budgets::transaction_is_open()', 'audit-budget-transaction-state')
...
for forbidden_probe in ('@@session.in_transaction', '@@in_transaction'):
    forbid(audit, forbidden_probe, 'audit-no-nonportable-transaction-probe')
...
require(approvals, 'public static function authorize_exact', 'approval-readonly-authorizer')
...
require(overrides, "'mad4b/mutation-undo'", 'undo-registration')
...
require(servers, "'mad4b/mutation-undo'", 'undo-server-mount')
