...
$intent_indexes = $wpdb->get_results( "SHOW INDEX FROM {$t['intent_relations']}", ARRAY_A );
$index_state = array();
foreach ( is_array( $intent_indexes ) ? $intent_indexes : array() as $index_row ) {
    $name = (string) ( $index_row['Key_name'] ?? '' );
    if ( '' === $name ) continue;
    if ( ! isset( $index_state[ $name ] ) ) {
        $index_state[ $name ] = array( 'unique' => 0 === (int) ( $index_row['Non_unique'] ?? 1 ) );
    }
}
if ( empty( $index_state['relation_revision']['unique'] )
    || empty( $index_state['current_relation_key']['unique'] )
    || ! isset( $index_state['owner_scope_key'] )
    || ! empty( $index_state['owner_scope_key']['unique'] ) ) {
    fwrite( STDERR, wp_json_encode( array(
        'stage' => 'verify_intent_authority_indexes_after_upgrade',
        'source_version' => $from,
        'indexes' => $index_state,
    ), JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) . PHP_EOL );
    exit( 27 );
...
scenarios=(
  ...
  "10:864f69d9bb3016c9fafbd4fd481aea9b778b5862"
)
...
echo
echo "mad4b.schema-mariadb-v6-v7-v8-v9-v10-to-v11.integration.v1: PASS"
