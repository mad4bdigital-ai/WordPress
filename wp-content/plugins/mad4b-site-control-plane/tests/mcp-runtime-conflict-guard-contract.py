...
for forbidden in (
    ...
    'wp_remote_post(', 'curl_exec(', 'file_put_contents(', 'unlink(', 'rename(',
    'active_sitewide_plugins', 'switch_to_blog(',
):
...
for marker in (
    ...
    'Hostinger bundled adapter active', 'Official loads before Hostinger',
    'Collision risk detected', 'Load-order repair applied', 'Next request required',
):
    require(diagnostics, marker, 'diagnostics-evidence')

print('mad4b.site-control-plane.mcp-runtime-conflict-guard-contract.v1: PASS')
