...
final class MAD4B_SCP_Servers {
	...
	public static function governance_gated_write_tools() {
		return array(
			array( 'ability' => 'mad4b/context-ai-review', 'provider' => 'core', 'gate_class' => 'governance', 'reason' => 'ai_review_standing_delegation_not_eligible' ),
		);
	}
}
...
mad4b_projection_assert( 1 === (int) $p['governance_gated_write_tool_count'], 'governance-gated count drifted', $p );
...
mad4b_projection_assert( 2 === count( $p['blocked_catalog_abilities'] ), 'provider-gated catalog abilities were not preserved', $p );
...
mad4b_projection_assert( in_array( 'governance_gated_runtime_overlap', $authority_overlap['consistency_violations'], true ), 'governance-gated/runtime overlap was not detected', $authority_overlap );
mad4b_projection_assert( 'projection_inconsistent' === $authority_overlap['state'], 'governance-gated/runtime overlap must fail closed', $authority_overlap );
