...
final class FakeCanaryWrapperAdapter {
	...
	public function ability_names() { return array( 'read' => array(), 'content' => array(), 'admin' => array( 'mad4b/provider-canary-execute' ) ); }
...
final class FakeBitFlowsAdapter {
	...
	public function ability_names() { return array( 'read' => array(), 'content' => array(), 'admin' => array( 'bitflows/run-flow' ) ); }
...
expect_true( in_array( 'mad4b/provider-canary-execute', $projection['eligible'], true ), 'core canary wrapper must mount through the canonical adapter compiler without provider certification' );
...
expect_true( 'core' === MAD4B_SCP_Servers::provider_for_ability( 'mad4b-write', 'mad4b/provider-canary-execute' ), 'canary wrapper provider must resolve to core' );
