...
Normal `developer-wp-cli` is a finite read/verification surface, not a mutation denylist. It denies WP-CLI aliases, caller-supplied `--path`, and global escape/bootstrap flags such as `--exec`, `--require`, `--ssh` and `--http`; any command family not explicitly allowlisted is denied and must use a dedicated governed ability or Developer Breakglass.
