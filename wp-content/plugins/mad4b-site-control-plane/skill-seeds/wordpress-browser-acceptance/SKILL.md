...
10. Collect evidence only through the package-owned passive observer and complete result identity/order observation required by the signed case. Provider attempt logs must redact tokens, API keys, bearer values, and credential-bearing WebSocket query parameters.
11. Submit the exact evidence envelope back through `mad4b/browser-acceptance-result` with the original `plan_digest` and `plan_signature`. Do not self-declare PASS from the external runner.
12. Treat `browser_runtime_parity_verified=true` and reducer `verdict=PASS` as browser evidence only. SEO publication, Production activation, write authority, plugin lifecycle mutation, and other release gates remain independent.
13. Return the selected provider, provider-attempt ledger, run-budget usage, plan digest, reducer verdict, parity verification, unresolved observation gaps, and the next governed release gate.
