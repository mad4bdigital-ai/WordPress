...
15. Re-read `context/brand-core-coverage`. Ready means the exact category is present as approved Brand Authority with matching reviewed content hash, generation evidence is fresh (or carries the explicit human override), and there is exactly one effective approved content hash for the category. This is the **single effective Brand Authority** rule: conflicting approved content hashes keep the category blocked.
