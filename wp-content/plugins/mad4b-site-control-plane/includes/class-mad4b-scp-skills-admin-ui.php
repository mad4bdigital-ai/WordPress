...
final class MAD4B_SCP_Skills_Admin_UI {
	...
	public static function render_page() {
		...
		if ( isset( $_POST['mad4b_skill_action'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- verified below.
			$action = sanitize_key( wp_unslash( $_POST['mad4b_skill_action'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
			if ( 'export' === $action ) self::handle_export();
		}
	...
	private static function render_status( array $status ) {
		...
		if ( empty( $status['editor_enabled'] ) ) {
			...
			echo esc_html__( 'On Staging, define MAD4B_SKILLS_EDITOR_ENABLED=true to enable the local admin editor. Production additionally requires MAD4B_SKILLS_PRODUCTION_EDITOR_ENABLED=true. These switches do not enable MCP mutation.', 'mad4b-site-control-plane' );
			echo '</p></div>';
		}
	}
	...
	private static function render_editor( array $status, $selected ) {
		$level = is_array( $selected ) ? $selected['level'] : 'site';
		$target = is_array( $selected ) ? $selected['target'] : '';
		$name = is_array( $selected ) ? $selected['name'] : '';
		$description = is_array( $selected ) ? $selected['description'] : '';
		$body = '';
		if ( is_array( $selected ) && isset( $selected['content'] ) ) {
			$body = preg_replace( '/\A---\R.*?\R---\R/s', '', (string) $selected['content'], 1 );
			$body = trim( (string) $body );
		}
		$enabled = ! is_array( $selected ) || ! empty( $selected['enabled'] );

		echo '<h2>' . esc_html( is_array( $selected ) ? __( 'Edit Skill', 'mad4b-site-control-plane' ) : __( 'Create Skill', 'mad4b-site-control-plane' ) ) . '</h2>';
		...
		echo '<tr><th><label for="mad4b_skill_target">Target</label></th><td><input class="regular-text" id="mad4b_skill_target" name="target" value="' . esc_attr( 'site' === $level ? '' : $target ) . '"><p class="description">For provider use a stable plugin slug such as elementor or jet-engine. Site level ignores this field.</p></td></tr>';
		echo '<tr><th><label for="mad4b_skill_name">Name</label></th><td><input class="regular-text" required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" id="mad4b_skill_name" name="name" value="' . esc_attr( $name ) . '"><p class="description">Globally unique kebab-case name used in portable skills/&lt;name&gt;/SKILL.md.</p></td></tr>';
		...
		echo '</tbody></table>';
		submit_button( is_array( $selected ) ? __( 'Save Skill', 'mad4b-site-control-plane' ) : __( 'Create Skill', 'mad4b-site-control-plane' ), 'primary', 'submit', true, array( 'disabled' => empty( $status['editor_enabled'] ) ? 'disabled' : null ) );
		echo '</form>';
	}
	...
	private static function render_export( array $status ) {
		...
		echo '<p>' . esc_html__( 'Exports enabled runtime skills into a portable Agent Plugins ZIP with root plugin.json and skills/. If MAD4B_OPENAI_PLUGIN_APP_ID is configured, the ZIP also contains .app.json pointing to that registered ChatGPT MCP App. The exported capability remains Read.', 'mad4b-site-control-plane' ) . '</p>';
	...
	private static function handle_save_resource() {
		...
		$result = MAD4B_SCP_Skill_Registry::save_text_resource(
		...
		return is_wp_error( $result ) ? $result : __( 'Supporting Skill file saved.', 'mad4b-site-control-plane' );
	}
	...
	private static function handle_export() {
		...
		if ( is_wp_error( $export ) ) return;
		$path = $export['path'];
		$filename = $export['filename'];
		if ( headers_sent() || ! is_file( $path ) ) { @unlink( $path ); return; }
		nocache_headers();
		header( 'Content-Type: application/zip' );
		header( 'Content-Disposition: attachment; filename="' . str_replace( '"', '', $filename ) . '"' );
		header( 'Content-Length: ' . (string) filesize( $path ) );
		header( 'X-MAD4B-SHA256: ' . $export['sha256'] );
