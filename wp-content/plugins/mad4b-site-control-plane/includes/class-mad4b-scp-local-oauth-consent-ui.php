...
final class MAD4B_SCP_Local_OAuth_Consent_UI {
	const CONTRACT = 'mad4b.local-oauth-consent-ui.v2';
	...
	public static function enhance_document( $html ) {
		...
		$css = '<style id="' . self::STYLE_ID . '">'
			...
			. 'main:after{content:"OAuth identity/read scope · write authority separate · PKCE S256";display:block;margin-top:20px;padding-top:16px;border-top:1px solid #f1f5f9;color:var(--m4-muted);font-size:12px;line-height:1.5;}'
			. '@media(max-width:520px){main{width:min(calc(100% - 20px),620px)!important;margin:10px auto!important;padding:24px!important;border-radius:16px;}h1{font-size:25px}.mad4b-grant-metrics{grid-template-columns:1fr}.mad4b-grant-head{align-items:flex-start;flex-direction:column}form{align-items:stretch;flex-direction:column}button{width:100%}}'
