...
final class MAD4B_SCP_Local_OAuth_Consent_UI {
	...
	public static function enhance_document( $html ) {
		...
		if ( false !== strpos( $html, '<section class="mad4b-live-grants"' ) ) {
			$html = str_replace( '<section class="mad4b-live-grants"', $context . '<section class="mad4b-live-grants"', $html );
		}
		...
		$css = '<style id="' . self::STYLE_ID . '">'
			...
			. '@media(max-width:520px){main{width:min(calc(100% - 20px),620px)!important;margin:10px auto!important;padding:24px!important;border-radius:16px;}h1{font-size:25px}.mad4b-consent-grid,.mad4b-grant-metrics{grid-template-columns:1fr}.mad4b-grant-head{align-items:flex-start;flex-direction:column}form{align-items:stretch;flex-direction:column}button{width:100%}}'
