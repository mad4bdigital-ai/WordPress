...
final class MAD4B_SCP_Local_OAuth_Consent_UI {
	const CONTRACT = 'mad4b.local-oauth-consent-ui.v4';
	...
	public static function enhance_document( $html ) {
		...
		$step_up_request = 'is requesting OAuth read access plus a governed Staging authority step-up scope on this same WordPress MCP resource. The step-up scope permits requesting the composite Full Staging Authority operation only; it does not itself create Write, Developer, Developer Breakglass, Production, or raw-SQL Breakglass authority.';
		...
		$context = '<section class="mad4b-consent-context" aria-label="OAuth consent boundary">'
			...
			. ( $step_up_requested ? '<p><code>mad4b:authority:step-up</code> is a request permission for the exact composite Staging convergence flow. It is not a write grant and cannot bypass exact plan matching, enrolled administrator identity, confirmation, audit readiness, or fail-closed governance.</p>' : '' )
			. '</section>';
		...
		$html = str_replace( '>Approve</button>', $step_up_requested ? '>Approve governed access</button>' : '>Approve read access</button>', $html );
		...
		$css = '<style id="' . self::STYLE_ID . '">'
			...
			. 'main:after{order:30;content:"OAuth identity · Step-up is request permission only · Write/Developer authorities separate · PKCE S256";display:block;margin-top:20px;padding-top:16px;border-top:1px solid #f1f5f9;color:var(--m4-muted);font-size:12px;line-height:1.5;}'
