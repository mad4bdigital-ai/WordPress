...
final class MAD4B_SCP_Live_Acceptance_Observer {
	...
	public static function register_abilities() {
		...
		self::add_read_ability( 'mad4b/snapshot-verify', 'Compare Client Snapshot Identity', array( __CLASS__, 'snapshot_verify' ), array( 'type' => 'object', 'properties' => array( 'client_snapshot_token' => array( 'type' => 'string', 'maxLength' => 80 ) ), 'required' => array( 'client_snapshot_token' ), 'additionalProperties' => false ) );
		self::add_read_ability( 'mad4b/live-acceptance-status', 'Get Aggregate Live Acceptance Status', array( __CLASS__, 'live_acceptance_status' ), array( 'type' => 'object', 'properties' => array( 'client_snapshot_token' => array( 'type' => 'string', 'maxLength' => 80 ) ), 'additionalProperties' => false ) );
	}
	...
	public static function observe_doing_it_wrong( $function_name, $message, $error_level = null ) { self::record_warning( 'doing_it_wrong', $function_name, $message ); }
	public static function observe_deprecated_function( $function_name, $replacement = null, $version = null ) { $message = 'Deprecated function ' . (string) $function_name; if ( is_string( $replacement ) && '' !== $replacement ) $message .= '; replacement=' . $replacement; self::record_warning( 'deprecated_function', $function_name, $message ); }
	public static function observe_deprecated_argument( $function_name, $message = null, $version = null ) { self::record_warning( 'deprecated_argument', $function_name, (string) $message ); }
	public static function observe_deprecated_hook( $hook_name, $replacement = null, $version = null, $message = null ) { self::record_warning( 'deprecated_hook', $hook_name, (string) $message ); }
	...
	private static function record_warning( $event_type, $function_name, $message ) {
		...
		$telemetry['events'][] = array( 'type' => sanitize_key( (string) $event_type ), 'classification' => $bucket, 'severity' => isset( $classification['severity'] ) ? $classification['severity'] : 'observed', 'function' => self::safe_identifier( $function_name ), 'message' => self::sanitize_warning_message( $message ), 'component' => isset( $classification['component'] ) ? $classification['component'] : '', 'plugin_slug' => isset( $classification['plugin_slug'] ) ? $classification['plugin_slug'] : '', 'lifecycle_phase' => did_action( 'init' ) ? 'post_init' : 'pre_init', 'request_class' => self::request_class(), 'observed_at' => gmdate( 'Y-m-d H:i:s' ), 'build_fingerprint' => self::current_build_fingerprint(), 'callers' => isset( $classification['callers'] ) ? $classification['callers'] : array() );
	...
	private static function query_monitor_bucket_events( array $telemetry, $bucket ) {
		...
		if ( isset( $telemetry['events_by_bucket'][ $bucket ] ) && is_array( $telemetry['events_by_bucket'][ $bucket ] ) ) {
			return array_slice( $telemetry['events_by_bucket'][ $bucket ], -1 * self::MAX_EVENTS );
		}
		$retained = array();
		$events = isset( $telemetry['events'] ) && is_array( $telemetry['events'] ) ? $telemetry['events'] : array();
		foreach ( $events as $event ) {
			if ( ! is_array( $event ) || $bucket !== sanitize_key( isset( $event['classification'] ) ? (string) $event['classification'] : '' ) ) continue;
			$retained[] = $event;
		}
		return array_slice( $retained, -1 * self::MAX_EVENTS );
	}

	private static function query_monitor_retention_status( array $telemetry, array $mad4b ) {
		$mad4b_events = self::query_monitor_bucket_events( $telemetry, 'mad4b' );
		$retained_doing_it_wrong = 0;
		foreach ( $mad4b_events as $event ) {
			if ( is_array( $event ) && 'doing_it_wrong' === sanitize_key( isset( $event['type'] ) ? (string) $event['type'] : '' ) ) $retained_doing_it_wrong++;
		}
		$counter_doing_it_wrong = isset( $mad4b['doing_it_wrong'] ) ? max( 0, (int) $mad4b['doing_it_wrong'] ) : 0;
		return array(
			'contract' => 'mad4b.query-monitor-event-retention.v1',
			'global_event_limit' => self::MAX_EVENTS,
			'bucket_event_limit' => self::MAX_EVENTS,
			'events_by_bucket_available' => isset( $telemetry['events_by_bucket'] ) && is_array( $telemetry['events_by_bucket'] ),
			'mad4b_retained_event_count' => count( $mad4b_events ),
			'mad4b_doing_it_wrong_counter_count' => $counter_doing_it_wrong,
			'mad4b_doing_it_wrong_retained_count' => $retained_doing_it_wrong,
			'mad4b_doing_it_wrong_detail_gap_count' => max( 0, $counter_doing_it_wrong - $retained_doing_it_wrong ),
			'mad4b_doing_it_wrong_details_complete' => $counter_doing_it_wrong <= $retained_doing_it_wrong,
		);
	}
	...
	public static function build_provenance_status() {
		...
		if ( '' === $path || ! is_readable( $path ) ) return $base; $base['manifest_present'] = true; $raw = file_get_contents( $path ); $data = is_string( $raw ) ? json_decode( $raw, true ) : null;
		...
		foreach ( array( 'build_fingerprint', 'package_manifest_digest' ) as $key ) if ( empty( $data[ $key ] ) || ! preg_match( '/^[a-f0-9]{64}$/', (string) $data[ $key ] ) ) { $base['provenance_mismatch'] = array( 'manifest_hash_invalid' ); return $base; }
	...
	public static function inventory_attestation_from_names( array $external_names, $captured_build_fingerprint = '' ) {
		...
		// Dynamic provider eligibility/certification is projected when the stored
		// attestation is read. Keep tools/list itself bound only to transport and
		// the stable logical write catalog.
	...
	private static function ability_to_mcp_tool_name( $ability_name ) { if ( ! class_exists( '\\WP\\MCP\\Domain\\Utils\\McpNameSanitizer' ) ) return ''; $name = \WP\MCP\Domain\Utils\McpNameSanitizer::sanitize_name( (string) $ability_name ); return is_wp_error( $name ) || ! is_string( $name ) ? '' : trim( $name ); }
	private static function abilities_to_mcp_names( array $abilities ) { $names = array(); foreach ( $abilities as $ability ) { $name = self::ability_to_mcp_tool_name( $ability ); if ( '' === $name ) return array(); $names[] = $name; } return self::normalize_tool_names( $names ); }
	...
	private static function expected_write_tool_names() { if ( ! class_exists( 'MAD4B_SCP_Servers' ) ) return array(); $abilities = method_exists( 'MAD4B_SCP_Servers', 'external_write_tools' ) ? MAD4B_SCP_Servers::external_write_tools() : MAD4B_SCP_Servers::write_tools(); return self::abilities_to_mcp_names( $abilities ); }
	private static function eligible_write_tool_names() { return class_exists( 'MAD4B_SCP_Servers' ) ? self::abilities_to_mcp_names( MAD4B_SCP_Servers::write_tools() ) : array(); }
	private static function blocked_write_tool_names() { if ( ! class_exists( 'MAD4B_SCP_Servers' ) ) return array(); $abilities = array(); foreach ( MAD4B_SCP_Servers::blocked_write_tools() as $entry ) if ( is_array( $entry ) && isset( $entry['ability'] ) ) $abilities[] = $entry['ability']; return self::abilities_to_mcp_names( $abilities ); }
	...
	private static function external_capture_allowed( $request ) { if ( ! self::staging_capture_allowed() || ! defined( 'REST_REQUEST' ) || true !== REST_REQUEST || ( defined( 'WP_CLI' ) && WP_CLI ) || ( defined( 'DOING_CRON' ) && DOING_CRON ) ) return false; if ( ! is_object( $request ) || ! method_exists( $request, 'get_route' ) ) return false; $route = '/' . ltrim( rtrim( (string) $request->get_route(), '/' ), '/' ); return '/mcp/' . self::SERVER_ID === $route && class_exists( 'MAD4B_SCP_OAuth_Resource_Bridge' ) && MAD4B_SCP_OAuth_Resource_Bridge::verified_bearer_active(); }
	private static function verified_request_context( $request ) { $identity = class_exists( 'MAD4B_SCP_Identity_Context' ) ? MAD4B_SCP_Identity_Context::current() : null; $oauth = class_exists( 'MAD4B_SCP_OAuth_Resource_Bridge' ) ? MAD4B_SCP_OAuth_Resource_Bridge::status() : array(); if ( ! is_array( $identity ) || empty( $identity['authenticated'] ) || 'oauth2_bearer' !== (string) $identity['auth_method'] || empty( $identity['subject_fingerprint'] ) ) return null; $scopes = isset( $identity['token_scopes'] ) && is_array( $identity['token_scopes'] ) ? array_values( array_unique( array_map( 'sanitize_text_field', $identity['token_scopes'] ) ) ) : array(); if ( ! in_array( self::REQUIRED_SCOPE, $scopes, true ) ) return null; $claims = self::verified_jwt_claims( $request ); if ( ! is_array( $claims ) ) return null; $issuer = isset( $oauth['issuer'] ) ? rtrim( (string) $oauth['issuer'], '/' ) : ''; $resource = isset( $oauth['resource'] ) ? untrailingslashit( (string) $oauth['resource'] ) : ''; $client_id = isset( $claims['client_id'] ) ? trim( (string) $claims['client_id'] ) : ''; if ( '' === $issuer || '' === $resource || empty( $claims['iss'] ) || empty( $claims['resource'] ) || ! hash_equals( $issuer, rtrim( (string) $claims['iss'], '/' ) ) || ! hash_equals( $resource, untrailingslashit( (string) $claims['resource'] ) ) || ! hash_equals( self::CHATGPT_CLIENT_ID, $client_id ) ) return null; return array( 'environment' => ( class_exists( 'MAD4B_SCP_Site_Profile' ) ? MAD4B_SCP_Site_Profile::current_environment() : ( function_exists( 'wp_get_environment_type' ) ? sanitize_key( (string) wp_get_environment_type() ) : 'unknown' ) ), 'resource' => $resource, 'issuer' => $issuer, 'client_id' => $client_id, 'auth_method' => 'oauth2_bearer', 'wp_user_id' => isset( $identity['wp_user_id'] ) ? absint( $identity['wp_user_id'] ) : 0, 'subject_fingerprint' => strtolower( (string) $identity['subject_fingerprint'] ), 'scope_set' => $scopes ); }
	private static function verified_jwt_claims( $request ) { $authorization = method_exists( $request, 'get_header' ) ? trim( (string) $request->get_header( 'authorization' ) ) : ''; if ( ! preg_match( '/^Bearer\s+([^\s]+)$/i', $authorization, $matches ) ) return null; $parts = explode( '.', $matches[1] ); unset( $authorization, $matches ); if ( 3 !== count( $parts ) || strlen( $parts[1] ) > 32768 ) return null; $payload = self::base64url_decode( $parts[1] ); unset( $parts ); if ( ! is_string( $payload ) || '' === $payload || strlen( $payload ) > 24576 ) return null; $claims = json_decode( $payload, true ); unset( $payload ); return is_array( $claims ) ? $claims : null; }
	...
	private static function response_session_id( $response ) { foreach ( $response->get_headers() as $name => $value ) if ( 'mcp-session-id' === strtolower( (string) $name ) ) { $value = is_array( $value ) ? reset( $value ) : $value; $value = is_string( $value ) ? trim( $value ) : ''; return strlen( $value ) <= 512 ? $value : ''; } return ''; }
