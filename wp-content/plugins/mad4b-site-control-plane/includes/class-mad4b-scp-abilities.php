...
final class MAD4B_SCP_Abilities {
		...
		try {
			wp_register_ability(
				...
				array(
					...
					'description' => 'Execute only Remote Operation Parity abilities explicitly mounted on the bounded Staging enrollment surface. This is not a generic admin or governed-write dispatcher.',
					...
					'input_schema' => $this->schema(
						array(
							'ability_name' => array( 'type' => 'string', 'minLength' => 3, 'maxLength' => 180 ),
							'expected_input_schema_sha256' => array( 'type' => 'string', 'minLength' => 64, 'maxLength' => 64, 'pattern' => '^[A-Fa-f0-9]{64}		$schema = array( 'type' => 'object', 'properties' => $properties, 'additionalProperties' => false );
	...
	private function governed_enrollment_target( $ability_name ) {
		...
		if ( in_array( $ability_name, array( 'mad4b/enrollment-info', 'mad4b/enrollment-execute' ), true ) ) return new WP_Error( 'mad4b_enrollment_dispatch_recursion_denied', 'Nested enrollment-dispatch execution is not allowed.' );
		...
		if ( ! array_key_exists( 'readonly', $annotations ) || false !== $annotations['readonly'] ) return new WP_Error( 'mad4b_enrollment_dispatch_read_target_denied', 'Only abilities explicitly annotated readonly=false may be selected through mad4b/enrollment-execute.' );
		if ( 'enrollment' !== ( isset( $mcp['surface'] ) ? (string) $mcp['surface'] : '' ) ) return new WP_Error( 'mad4b_enrollment_dispatch_surface_mismatch', 'Requested ability is not explicitly bound to the enrollment surface.' );
		return $ability;
	}
	...
	public function can_enrollment_dispatch( $input = null ) {
		...
		if ( ! class_exists( 'MAD4B_SCP_Remote_Operation_Parity' ) || ! method_exists( 'MAD4B_SCP_Remote_Operation_Parity', 'can_execute' ) ) return new WP_Error( 'mad4b_enrollment_dispatch_policy_unavailable', 'Remote Operation Parity execution policy is unavailable.' );
	...
	public function enrollment_execute( $input ) {
		...
		if ( is_wp_error( $ability ) ) return $ability;
		$actual_schema_sha256 = $this->ability_input_schema_sha256( $ability );
		$expected_schema_sha256 = strtolower( trim( (string) ( isset( $input['expected_input_schema_sha256'] ) ? $input['expected_input_schema_sha256'] : '' ) ) );
		if ( ! preg_match( '/^[a-f0-9]{64}$/', $expected_schema_sha256 ) || ! hash_equals( strtolower( $actual_schema_sha256 ), $expected_schema_sha256 ) ) return new WP_Error( 'mad4b_enrollment_dispatch_schema_drift', 'Requested enrollment ability input schema changed after planning.', array( 'ability_name' => $ability_name, 'current_input_schema_sha256' => $actual_schema_sha256 ) );
		$params = array_key_exists( 'input', $input ) ? $input['input'] : null;
		$target_input_schema = method_exists( $ability, 'get_input_schema' ) ? $ability->get_input_schema() : null;
		if ( ( null === $target_input_schema || empty( $target_input_schema ) ) && is_array( $params ) && empty( $params ) ) $params = null;
		$result = $ability->execute( $params );
		if ( is_wp_error( $result ) ) return $result;
		$mutation_performed = is_array( $result ) && array_key_exists( 'mutation_performed', $result ) ? (bool) $result['mutation_performed'] : true;
		return array(
