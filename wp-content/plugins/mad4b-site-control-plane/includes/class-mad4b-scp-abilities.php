...
final class MAD4B_SCP_Abilities {
	...
	private function register_enrollment_dispatcher() {
		if ( function_exists( 'wp_has_ability' ) && wp_has_ability( 'mad4b/enrollment-execute' ) ) return;
		...
		try {
			wp_register_ability( 'mad4b/enrollment-execute', array(
				'label' => 'Execute Bounded Enrollment Ability',
				'description' => 'Execute only Remote Operation Parity abilities explicitly mounted on the bounded Staging enrollment surface.',
				'category' => 'mad4b-enrollment',
				'execute_callback' => array( $this, 'enrollment_execute' ),
				'permission_callback' => array( $this, 'can_enrollment_dispatch' ),
				'input_schema' => $this->schema( array(
					'ability_name' => array( 'type' => 'string', 'minLength' => 3, 'maxLength' => 180 ),
				...
				'meta' => array(
					'public' => false,
					'show_in_rest' => false,
					'mcp' => array( 'public' => false, 'type' => 'tool', 'surface' => 'enrollment-dispatch', 'generic_remote_admin' => false, 'production_mutation_allowed' => false ),
	...
	private function governed_enrollment_target( $ability_name ) {
		...
		if ( in_array( $ability_name, array( 'mad4b/enrollment-info', 'mad4b/enrollment-execute' ), true ) ) return new WP_Error( 'mad4b_enrollment_dispatch_recursion_denied', 'Nested enrollment dispatch is not allowed.' );
		if ( ! class_exists( 'MAD4B_SCP_Remote_Operation_Parity' ) || ! method_exists( 'MAD4B_SCP_Remote_Operation_Parity', 'enrollment_abilities' ) ) return new WP_Error( 'mad4b_enrollment_dispatch_allowlist_unavailable', 'Remote Operation Parity enrollment inventory is unavailable.' );
		$allowed = array_values( array_unique( array_map( 'strval', MAD4B_SCP_Remote_Operation_Parity::enrollment_abilities() ) ) );
		if ( ! in_array( $ability_name, $allowed, true ) ) return new WP_Error( 'mad4b_enrollment_dispatch_target_not_allowed', 'Requested ability is outside the bounded Remote Operation Parity enrollment allowlist.' );
		if ( ! function_exists( 'wp_has_ability' ) || ! function_exists( 'wp_get_ability' ) || ! wp_has_ability( $ability_name ) ) return new WP_Error( 'mad4b_enrollment_dispatch_target_unavailable', 'Requested enrollment ability is not registered.' );
		$ability = wp_get_ability( $ability_name );
		if ( ! is_object( $ability ) || ! method_exists( $ability, 'get_meta' ) || ! method_exists( $ability, 'execute' ) ) return new WP_Error( 'mad4b_enrollment_dispatch_contract_unavailable', 'Requested enrollment ability does not expose the required contract.' );
		$meta = $ability->get_meta();
		$annotations = isset( $meta['annotations'] ) && is_array( $meta['annotations'] ) ? $meta['annotations'] : array();
		$mcp = isset( $meta['mcp'] ) && is_array( $meta['mcp'] ) ? $meta['mcp'] : array();
		if ( ! array_key_exists( 'readonly', $annotations ) || false !== $annotations['readonly'] ) return new WP_Error( 'mad4b_enrollment_dispatch_read_target_denied', 'Only readonly=false enrollment abilities may be dispatched.' );
		if ( 'enrollment' !== ( isset( $mcp['surface'] ) ? (string) $mcp['surface'] : '' ) ) return new WP_Error( 'mad4b_enrollment_dispatch_surface_mismatch', 'Requested ability is not bound to the enrollment surface.' );
		return $ability;
	}

	public function can_enrollment_dispatch( $input = null ) {
		if ( ! class_exists( 'MAD4B_SCP_Site_Profile_Enrollment' ) || ! method_exists( 'MAD4B_SCP_Site_Profile_Enrollment', 'can_access_transport' ) ) return new WP_Error( 'mad4b_enrollment_dispatch_transport_unavailable', 'Bounded enrollment transport authority is unavailable.' );
		$transport = MAD4B_SCP_Site_Profile_Enrollment::can_access_transport( $input );
		if ( is_wp_error( $transport ) || ! $transport ) return $transport;
		if ( ! class_exists( 'MAD4B_SCP_Remote_Operation_Parity' ) || ! method_exists( 'MAD4B_SCP_Remote_Operation_Parity', 'can_execute' ) ) return new WP_Error( 'mad4b_enrollment_dispatch_policy_unavailable', 'Remote Operation Parity policy is unavailable.' );
		$policy = MAD4B_SCP_Remote_Operation_Parity::can_execute( $input );
		if ( is_wp_error( $policy ) || ! $policy ) return $policy;
		...
		return is_wp_error( $ability ) ? $ability : true;
	}
	...
	public function enrollment_info( $input ) {
		...
		return array(
			...
			'annotations' => isset( $meta['annotations'] ) && is_array( $meta['annotations'] ) ? $meta['annotations'] : array(),
			'authority_surface' => 'mad4b-enrollment', 'read_only' => true, 'mutation_performed' => false,
		);
	}
	...
	public function enrollment_execute( $input ) {
		...
		$expected = strtolower( trim( (string) ( isset( $input['expected_input_schema_sha256'] ) ? $input['expected_input_schema_sha256'] : '' ) ) );
		if ( ! preg_match( '/^[a-f0-9]{64}$/', $expected ) || ! hash_equals( strtolower( $actual ), $expected ) ) return new WP_Error( 'mad4b_enrollment_dispatch_schema_drift', 'Requested enrollment ability input schema changed after planning.', array( 'ability_name' => $ability_name, 'current_input_schema_sha256' => $actual ) );
		$params = array_key_exists( 'input', $input ) ? $input['input'] : null;
		$result = $ability->execute( $params );
		if ( is_wp_error( $result ) ) return $result;
		$performed = is_array( $result ) && array_key_exists( 'mutation_performed', $result ) ? (bool) $result['mutation_performed'] : true;
		return array( 'contract' => 'mad4b.chatgpt-enrollment-execute.v1', 'ability_name' => $ability_name, 'input_schema_sha256' => $actual, 'authority_surface' => 'mad4b-enrollment', 'result' => $result, 'mutation_performed' => $performed );
	}
