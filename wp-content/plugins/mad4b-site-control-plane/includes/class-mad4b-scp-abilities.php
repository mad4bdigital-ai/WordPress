...
final class MAD4B_SCP_Abilities {
	...
	private function enrollment_operation_record( $ability_name ) {
		$ability_name = trim( (string) $ability_name );
		...
		if ( ! class_exists( 'MAD4B_SCP_Remote_Operation_Parity' ) || ! method_exists( 'MAD4B_SCP_Remote_Operation_Parity', 'catalog' ) || ! method_exists( 'MAD4B_SCP_Remote_Operation_Parity', 'enrollment_abilities' ) ) return new WP_Error( 'mad4b_enrollment_dispatch_catalog_unavailable', 'Bounded enrollment operation catalog is unavailable.' );

		$mounted = array_values( array_unique( array_map( 'strval', MAD4B_SCP_Remote_Operation_Parity::enrollment_abilities() ) ) );
		if ( ! in_array( $ability_name, $mounted, true ) ) return new WP_Error( 'mad4b_enrollment_dispatch_target_not_mounted', 'Requested ability is not mounted on the bounded enrollment surface.' );
		...
		foreach ( MAD4B_SCP_Remote_Operation_Parity::catalog() as $operation_id => $row ) {
			...
			if ( 'mad4b-enrollment' !== ( isset( $row['authority_surface'] ) ? (string) $row['authority_surface'] : '' ) ) return new WP_Error( 'mad4b_enrollment_dispatch_surface_mismatch', 'Requested operation is not registered on the bounded enrollment authority surface.' );
			if ( 'operator' !== ( isset( $row['remote_caller_role'] ) ? (string) $row['remote_caller_role'] : '' ) ) return new WP_Error( 'mad4b_enrollment_dispatch_caller_role_denied', 'Requested enrollment operation is reserved for a different governed caller role.' );
			if ( ! empty( $row['human_decision_required'] ) ) return new WP_Error( 'mad4b_enrollment_dispatch_human_decision_denied', 'Human-decision operations cannot be executed through the generic ChatGPT enrollment dispatcher.' );
			if ( 'deny' !== ( isset( $row['production_policy'] ) ? (string) $row['production_policy'] : '' ) ) return new WP_Error( 'mad4b_enrollment_dispatch_production_policy_denied', 'Only production-denied enrollment operations may be dispatched from ChatGPT.' );
			if ( empty( $row['remote_registered'] ) || empty( $row['execution_eligible'] ) || empty( $row['remote_parity_ready'] ) ) return new WP_Error( 'mad4b_enrollment_dispatch_operation_not_ready', 'Requested enrollment operation is not currently remote-parity ready.' );
			$registration = isset( $row['registration_digest'] ) ? strtolower( trim( (string) $row['registration_digest'] ) ) : '';
			if ( 1 !== preg_match( '/^[a-f0-9]{64}$/', $registration ) ) return new WP_Error( 'mad4b_enrollment_dispatch_registration_invalid', 'Enrollment operation registration digest is unavailable or invalid.' );
			$row['operation_id'] = (string) $operation_id;
			$row['registration_digest'] = $registration;
			return $row;
		}
		return new WP_Error( 'mad4b_enrollment_dispatch_operation_unknown', 'Requested ability is not represented by a governed Remote Operation Parity catalog row.' );
	}

	private function enrollment_dispatch_policy_digest( $operation_id, array $row, $input_schema_sha256 ) {
		$payload = array(
			'contract' => 'mad4b.chatgpt-enrollment-dispatch.v2',
	...
	private function governed_enrollment_operation( $ability_name ) {
		$row = $this->enrollment_operation_record( $ability_name );
		...
		$schema_sha = $this->ability_input_schema_sha256( $ability );
		$row['input_schema_sha256'] = $schema_sha;
		$row['dispatch_policy_digest'] = $this->enrollment_dispatch_policy_digest( $row['operation_id'], $row, $schema_sha );
		return array( 'row' => $row, 'ability' => $ability );
	}

	private function governed_enrollment_target( $ability_name ) {
		$operation = $this->governed_enrollment_operation( $ability_name );
		return is_wp_error( $operation ) ? $operation : $operation['ability'];
	}
	...
	public function can_enrollment_dispatch( $input = null ) {
		...
		$operation = $this->governed_enrollment_operation( $input['ability_name'] );
		if ( is_wp_error( $operation ) ) return $operation;
		if ( ! class_exists( 'MAD4B_SCP_OAuth_Resource_Bridge' ) || ! MAD4B_SCP_OAuth_Resource_Bridge::verified_bearer_active() ) return new WP_Error( 'mad4b_enrollment_dispatch_bearer_required', 'Verified OAuth bearer identity is required.' );
		if ( ! MAD4B_SCP_OAuth_Resource_Bridge::verified_bearer_has_scope( MAD4B_SCP_OAuth_Resource_Bridge::AUTHORITY_STEP_UP_SCOPE ) ) return new WP_Error( 'mad4b_enrollment_dispatch_step_up_scope_required', 'Dedicated Staging authority step-up scope is required.' );
		if ( ! class_exists( 'MAD4B_SCP_Local_OAuth_Server' ) || ! MAD4B_SCP_OAuth_Resource_Bridge::verified_bearer_client_is( MAD4B_SCP_Local_OAuth_Server::CHATGPT_CIMD_CLIENT_ID ) ) return new WP_Error( 'mad4b_enrollment_dispatch_chatgpt_client_required', 'Enrollment dispatch requires OAuth attribution to the exact ChatGPT CIMD client.' );
		if ( ( class_exists( 'MAD4B_SCP_Policy' ) && MAD4B_SCP_Policy::can_breakglass() ) || ( defined( 'MAD4B_MCP_BREAKGLASS_ENABLED' ) && true === constant( 'MAD4B_MCP_BREAKGLASS_ENABLED' ) ) ) return new WP_Error( 'mad4b_enrollment_dispatch_breakglass_denied', 'Breakglass must remain disabled during bounded enrollment dispatch.' );
	...
	public function enrollment_discover( $input ) {
		...
		$items = array();
		$candidates = class_exists( 'MAD4B_SCP_Remote_Operation_Parity' ) && method_exists( 'MAD4B_SCP_Remote_Operation_Parity', 'enrollment_abilities' ) ? MAD4B_SCP_Remote_Operation_Parity::enrollment_abilities() : array();
		foreach ( array_values( array_unique( array_map( 'strval', $candidates ) ) ) as $ability_name ) {
			$operation = $this->governed_enrollment_operation( $ability_name );
			if ( is_wp_error( $operation ) ) continue;
			$row = $operation['row'];
			$ability = $operation['ability'];
			$label = method_exists( $ability, 'get_label' ) ? (string) $ability->get_label() : '';
			$description = method_exists( $ability, 'get_description' ) ? (string) $ability->get_description() : '';
			$category = method_exists( $ability, 'get_category' ) ? (string) $ability->get_category() : '';
			$haystack = strtolower( implode( ' ', array( $ability_name, $row['operation_id'], $label, $description, $category, isset( $row['provider'] ) ? (string) $row['provider'] : '', isset( $row['remote_mode'] ) ? (string) $row['remote_mode'] : '' ) ) );
			if ( '' !== $query && false === strpos( $haystack, $query ) ) continue;
			$items[] = array(
				...
				'remote_caller_role' => $row['remote_caller_role'],
		...
		return array(
			'contract' => 'mad4b.chatgpt-enrollment-discovery.v2',
			'query' => $query,
			'items' => $items,
			'count' => count( $items ),
	...
	public function enrollment_info( $input ) {
		...
		$meta = $ability->get_meta();
		return array(
			'contract' => 'mad4b.chatgpt-enrollment-ability-info.v2',
			'operation_id' => $row['operation_id'],
			'ability_name' => $ability_name,
			'label' => method_exists( $ability, 'get_label' ) ? (string) $ability->get_label() : '',
			'description' => method_exists( $ability, 'get_description' ) ? (string) $ability->get_description() : '',
	...
	public function enrollment_execute( $input ) {
		...
		$expected_registration = strtolower( trim( (string) $input['expected_registration_digest'] ) );
		$expected_policy = strtolower( trim( (string) $input['expected_dispatch_policy_digest'] ) );
		$expected_schema = strtolower( trim( (string) $input['expected_input_schema_sha256'] ) );

		if ( ! hash_equals( strtolower( (string) $row['registration_digest'] ), $expected_registration ) ) return new WP_Error( 'mad4b_enrollment_dispatch_registration_drift', 'Operation registration changed after planning.', array( 'current_registration_digest' => $row['registration_digest'] ) );
		if ( ! hash_equals( strtolower( (string) $row['dispatch_policy_digest'] ), $expected_policy ) ) return new WP_Error( 'mad4b_enrollment_dispatch_policy_drift', 'Enrollment dispatch policy changed after planning.', array( 'current_dispatch_policy_digest' => $row['dispatch_policy_digest'] ) );
		if ( ! hash_equals( strtolower( (string) $row['input_schema_sha256'] ), $expected_schema ) ) return new WP_Error( 'mad4b_enrollment_dispatch_schema_drift', 'Enrollment target input schema changed after planning.', array( 'current_input_schema_sha256' => $row['input_schema_sha256'] ) );
		...
		return array(
			'contract' => 'mad4b.chatgpt-enrollment-execute.v2',
			'operation_id' => $row['operation_id'],
			'ability_name' => $ability_name,
			'registration_digest' => $row['registration_digest'],
			'dispatch_policy_digest' => $row['dispatch_policy_digest'],
	...
	public function database_raw_query( $input ) {
		...
		return array( 'verb' => $verb, 'query_hash' => $hash, 'affected_rows' => (int) $result );
				...
				'input' => array( 'type' => 'object', 'default' => array() ),
			), array( 'ability_name', 'expected_registration_digest', 'expected_dispatch_policy_digest', 'expected_input_schema_sha256' )
