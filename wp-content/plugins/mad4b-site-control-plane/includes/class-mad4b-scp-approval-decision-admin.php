...
final class MAD4B_SCP_Approval_Decision_Admin {
	...
	/** @internal Pure evaluator used by regression tests and the live admin handler. */
	public static function validate_decision_for_test( array $request, array $ticket, array $binding, array $candidate, array $agent, $is_admin, $environment, $host, $now ) {
		...
		if ( empty( $binding ) || 'mad4b.approval-candidate-binding.v1' !== ( isset( $binding['contract'] ) ? (string) $binding['contract'] : '' ) ) return new WP_Error( 'mad4b_approval_decision_binding_missing', 'Ticket has no candidate binding from its planning request; create a new exact ticket on this build.' );
		foreach ( array(
			'ticket_id' => $ticket_id,
	...
	public static function decide( array $request ) {
		...
		$validated = self::validate_decision_for_test(
			$request,
			$ticket,
			is_array( $binding ) ? $binding : array(),
			$candidate,
		...
		return MAD4B_SCP_Approval_Tickets::decide_pending(
			...
			array( 'candidate_sha' => (string) $candidate['source_commit_sha'], 'build_fingerprint' => (string) $candidate['build_fingerprint'], 'contract' => self::CONTRACT )
		);
	}
	...
	public static function render_page() {
		...
		echo '<p>' . esc_html__( 'Human-only pending → approved/rejected transition. This page never executes the target mutation. Production, Breakglass, recovery, stale-build and unbound tickets fail closed.', 'mad4b-site-control-plane' ) . '</p>';
		...
		$rows = $wpdb->get_results( "SELECT a.*,g.slug AS agent_slug,g.label AS agent_label FROM {$t['approvals']} a LEFT JOIN {$t['agents']} g ON g.id = a.agent_id ORDER BY a.id DESC LIMIT " . self::MAX_ROWS, ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery
		...
		foreach ( $rows as $row ) {
			...
			$binding = MAD4B_SCP_Approval_Tickets::candidate_binding( $ticket_id );
			$binding_ok = is_array( $binding ) && ! empty( $binding['candidate_sha'] ) && hash_equals( (string) $candidate['source_commit_sha'], (string) $binding['candidate_sha'] ) && ! empty( $binding['build_fingerprint'] ) && hash_equals( (string) $candidate['build_fingerprint'], (string) $binding['build_fingerprint'] ) && ! empty( $binding['payload_sha256'] ) && hash_equals( $payload, (string) $binding['payload_sha256'] );
			...
			echo '<tr><td><code>' . esc_html( $ticket_id ) . '</code></td><td><code>' . esc_html( isset( $row['ability_name'] ) ? $row['ability_name'] : '' ) . '</code></td><td><strong>' . esc_html( isset( $row['status'] ) ? $row['status'] : '' ) . '</strong></td><td>' . esc_html( isset( $row['expires_at'] ) ? $row['expires_at'] : '' ) . '</td><td>' . esc_html( $binding_ok ? 'exact current build' : 'missing/stale' ) . '</td><td>';
			if ( $can_decide ) {
