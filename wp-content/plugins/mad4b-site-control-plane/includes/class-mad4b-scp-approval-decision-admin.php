 ...
 *
 * GET is a read-model only surface. It never reconciles write authority or primes
 * MCP. POST remains authoritative and revalidates schema, payload, candidate,
 * provider and ticket state before an administrator decision can be persisted.
 */
final class MAD4B_SCP_Approval_Decision_Admin {
	...
	public static function validate_decision_for_test( array $request, array $ticket, array $binding, array $candidate, array $agent, $is_admin, $environment, $host, $now ) {
		...
		if ( 'staging' !== (string) $environment || self::STAGING_HOST !== strtolower( rtrim( (string) $host, '.' ) ) ) return new WP_Error( 'mad4b_approval_decision_staging_only', 'Human approval decisions are restricted to the exact governed Staging origin.' );
		...
		if ( 'mutation' !== ( isset( $ticket['ticket_class'] ) ? sanitize_key( (string) $ticket['ticket_class'] ) : '' ) ) return new WP_Error( 'mad4b_approval_decision_class_denied', 'Breakglass and recovery tickets cannot be decided from the normal Staging approval console.' );
		...
		if ( empty( $agent ) || 'enabled' !== ( isset( $agent['status'] ) ? (string) $agent['status'] : '' ) || 'staging' !== ( isset( $agent['environment'] ) ? (string) $agent['environment'] : '' ) ) return new WP_Error( 'mad4b_approval_decision_agent_denied', 'Ticket is not bound to an enabled Staging agent.' );
		if ( class_exists( 'MAD4B_SCP_Staging_Write_Authority' ) && defined( 'MAD4B_SCP_Staging_Write_Authority::AGENT_SLUG' ) && ( empty( $agent['slug'] ) || MAD4B_SCP_Staging_Write_Authority::AGENT_SLUG !== (string) $agent['slug'] ) ) return new WP_Error( 'mad4b_approval_decision_agent_denied', 'Ticket is not bound to the dedicated governed Staging write agent.' );
		...
		if ( empty( $binding ) || MAD4B_SCP_Approval_Tickets::CANDIDATE_BINDING_CONTRACT !== ( isset( $binding['contract'] ) ? (string) $binding['contract'] : '' ) ) return new WP_Error( 'mad4b_approval_decision_binding_missing', 'Ticket has no candidate binding from its planning request; create a new exact ticket on this build.' );
		foreach ( array( 'ticket_id' => $ticket_id, 'payload_sha256' => $payload, 'candidate_sha' => $expected_sha, 'build_fingerprint' => $expected_build, 'environment' => 'staging', 'host' => self::STAGING_HOST ) as $key => $expected ) {
			if ( ! isset( $binding[ $key ] ) || ! hash_equals( (string) $expected, (string) $binding[ $key ] ) ) return new WP_Error( 'mad4b_approval_decision_binding_mismatch', 'Pending ticket was planned for a different candidate, payload, or environment.' );
		}
		return true;
	}

	private static function prepare_authority_runtime_after_validation() {
		if ( ! class_exists( 'MAD4B_SCP_Staging_Write_Authority' ) ) return new WP_Error( 'mad4b_approval_decision_write_authority_not_ready', 'Governed Staging write authority is unavailable.' );
		...
		if ( ! is_array( $status ) || empty( $status['ready'] ) || ! MAD4B_SCP_Staging_Write_Authority::effective() ) return new WP_Error( 'mad4b_approval_decision_write_authority_not_ready', 'Governed Staging write authority is not ready.' );
		return $status;
	}
	...
	public static function decide( array $request ) {
		...
		$validated = self::validate_decision_for_test( $request, $ticket, is_array( $binding ) ? $binding : array(), $candidate, is_array( $agent ) ? $agent : array(), current_user_can( 'manage_options' ), function_exists( 'wp_get_environment_type' ) ? sanitize_key( (string) wp_get_environment_type() ) : 'unknown', self::home_host(), time() );
		...
		return MAD4B_SCP_Approval_Tickets::decide_pending( $ticket_id, (string) $request['decision'], (string) $request['expected_payload_sha256'], array( 'candidate_sha' => (string) $candidate['source_commit_sha'], 'build_fingerprint' => (string) $candidate['build_fingerprint'], 'contract' => self::CONTRACT ) );
	}
	...
	public static function render_page() {
		...
		echo '<p>' . esc_html__( 'Human-only decision inbox. GET uses a read-optimized projection and never reconciles write authority; every POST revalidates the authoritative ticket, exact build and provider before changing state.', 'mad4b-site-control-plane' ) . '</p>';
		...
		echo '<p><strong>Candidate:</strong> <code>' . esc_html( $candidate['source_commit_sha'] ) . '</code><br><strong>Build:</strong> <code>' . esc_html( $candidate['build_fingerprint'] ) . '</code></p>';
		if ( ! MAD4B_SCP_Schema::is_ready() ) { echo '<div class="notice notice-error inline"><p>' . esc_html__( 'Current governance schema metadata is not ready. The decision inbox remains fail-closed until migration completes.', 'mad4b-site-control-plane' ) . '</p></div></div>'; return; }
		if ( empty( $candidate['ready'] ) ) { echo '<div class="notice notice-error inline"><p>' . esc_html__( 'Exact Staging build provenance is not ready. No ticket can be decided.', 'mad4b-site-control-plane' ) . '</p></div></div>'; return; }
		...
		echo '<a class="nav-tab ' . ( 'actionable' === $view ? 'nav-tab-active' : '' ) . '" href="' . esc_url( add_query_arg( 'view', 'actionable', $base_url ) ) . '">' . esc_html__( 'Needs action', 'mad4b-site-control-plane' ) . '</a>';
		echo '<a class="nav-tab ' . ( 'history' === $view ? 'nav-tab-active' : '' ) . '" href="' . esc_url( add_query_arg( 'view', 'history', $base_url ) ) . '">' . esc_html__( 'History', 'mad4b-site-control-plane' ) . '</a>';
		echo '</nav>';
		if ( 'history' === $view ) {
			...
			if ( empty( $rows ) ) echo '<p>' . esc_html__( 'No fresh exact-build approval ticket currently needs a human decision.', 'mad4b-site-control-plane' ) . '</p>';
	...
	private static function render_table( array $rows, array $candidate, $allow_decisions ) {
		...
		echo '<table class="widefat striped"><thead><tr><th>Ticket</th><th>Ability</th><th>Status</th><th>Expires</th><th>Candidate binding</th><th>Decision</th></tr></thead><tbody>';
		foreach ( $rows as $row ) {
			...
			echo '<tr><td><code>' . esc_html( $ticket_id ) . '</code></td><td><code>' . esc_html( isset( $row['ability_name'] ) ? $row['ability_name'] : '' ) . '</code></td><td><strong>' . esc_html( $effective ) . '</strong></td><td>' . esc_html( isset( $row['expires_at'] ) ? $row['expires_at'] : '' ) . '</td><td>' . esc_html( $binding_ok ? 'exact current build' : 'missing/stale' ) . '</td><td>';
