...
abstract class MAD4B_SCP_Adapter_Base {
	...
	public function register_category() {
		wp_register_ability_category(
			'mad4b-' . $this->id(),
			array(
				'label'       => 'MAD4B ' . $this->label(),
				'description' => 'Governed ' . $this->label() . ' integration abilities.',
			)
		);
	}
	...
	protected function add_ability( $name, $label, $method, $permission, $input_schema = null, $surface = 'read', $readonly = true, $destructive = false, $idempotent = true ) {
		$args = array(
			...
			'meta'                => array(
				'public'       => false,
				'show_in_rest' => false,
				'mcp'          => array( 'public' => true, 'type' => 'tool', 'surface' => $surface ),
				'annotations'  => array(
	...
	protected function unavailable_error() {
		return new WP_Error( 'mad4b_adapter_unavailable', $this->label() . ' is not active or its expected runtime contract is unavailable.' );
	}
