...
final class MAD4B_SCP_WooCommerce_Adapter extends MAD4B_SCP_Adapter_Base {
	...
	public function register_abilities() {
		...
		), array( 'product_id', 'fields' ) ), 'content', false, false, true );
	}
	...
	public function get_product( $input ) {
		...
		return array( 'product' => array(
			'id' => $product->get_id(), 'type' => $product->get_type(), 'name' => $product->get_name(), 'status' => $product->get_status(), 'sku' => $product->get_sku(), 'regular_price' => $product->get_regular_price(), 'sale_price' => $product->get_sale_price(), 'stock_status' => $product->get_stock_status(), 'featured' => $product->get_featured(), 'permalink' => $product->get_permalink(),
		) );
	}
	public function update_product( $input ) {
		...
		try {
			foreach ( $input['fields'] as $field => $value ) {
				...
				if ( 'status' === $field && ! in_array( $value, array( 'draft', 'pending', 'private', 'publish' ), true ) ) return new WP_Error( 'mad4b_wc_status_denied', 'Unsupported product status.' );
				...
				if ( 'featured' === $field ) $value = (bool) $value; else $value = sanitize_text_field( $value );
		...
		MAD4B_SCP_Audit::record( 'woocommerce/update-product', array( 'product_id' => $product->get_id(), 'fields' => array_keys( $input['fields'] ) ) );
		return array( 'product_id' => $saved, 'updated' => true );
	}
}
