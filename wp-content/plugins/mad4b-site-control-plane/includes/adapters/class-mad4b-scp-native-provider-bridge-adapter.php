 ...
 * authority without exposing a generic execute-anything surface.
 *
 * JetEngine development operations are fixed semantic wrappers. Import/export
 * wrappers are limited to native abilities that explicitly identify themselves
 * as import/export operations and are exact-bound by native ability + schema hash.
 */
final class MAD4B_SCP_Native_Provider_Bridge_Adapter extends MAD4B_SCP_Adapter_Base {
	...
	public function register_abilities() {
		...
		$map = array(
			...
			'jetengine/export-configuration' => array( 'jetengine_export_configuration', 'export_configuration' ),
		);
		foreach ( $map as $ability => $pair ) $this->add_ability( $ability, ucwords( str_replace( array( 'jetengine/','-' ), array( 'JetEngine ',' ' ), $ability ) ), $pair[0], $admin, $this->native_call_schema( true ), 'write', false, true, false );
		...
		$this->add_ability( 'mad4b/provider-export-content', 'Provider Native Content Export', 'provider_export_content', $admin, $this->provider_call_schema(), 'write', false, true, false );
	}

	private function native_call_schema( $require_binding ) {
		$props = array(
			'input' => array( 'type' => 'object', 'additionalProperties' => true, 'default' => array() ),
			'expected_native_ability' => array( 'type' => 'string', 'maxLength' => 191 ),
			'expected_schema_sha256' => array( 'type' => 'string', 'minLength' => 64, 'maxLength' => 64, 'pattern' => '^[a-f0-9]{64}$' ),
		);
		return $this->schema( $props, $require_binding ? array( 'expected_native_ability', 'expected_schema_sha256', 'input' ) : array( 'expected_native_ability', 'expected_schema_sha256' ) );
	}
	...
	private function ability_row( $name, $ability ) {
		$meta = $this->ability_meta( $ability ); $annotations = isset( $meta['annotations'] ) && is_array( $meta['annotations'] ) ? $meta['annotations'] : array();
		return array(
			'name' => (string) $name,
			...
			'idempotent' => isset( $annotations['idempotent'] ) ? (bool) $annotations['idempotent'] : null,
	...
	private function native_rows() {
		$rows = array(); foreach ( wp_get_abilities() as $name => $ability ) { $name = (string) $name; if ( 0 === strpos( $name, 'mad4b/' ) ) continue; if ( in_array( $name, $this->ability_names()['write'], true ) || in_array( $name, $this->ability_names()['read'], true ) ) continue; $rows[ $name ] = $this->ability_row( $name, $ability ); } ksort( $rows, SORT_STRING ); return $rows;
	}
	private function normalize_text( $text ) { $text = strtolower( wp_strip_all_tags( (string) $text ) ); $text = preg_replace( '/[^a-z0-9]+/', ' ', $text ); return trim( preg_replace( '/\s+/', ' ', $text ) ); }
	...
	private function jetengine_row( array $row ) { $text = $this->row_text( $row ); return false !== strpos( $text, 'jetengine' ) || false !== strpos( $text, 'jet engine' ) || false !== strpos( $text, 'crocoblock' ); }

	private function operation_matches( $operation, array $row ) {
		$text = $this->row_text( $row ); if ( false !== strpos( $operation, 'jetengine_' ) && ! $this->jetengine_row( $row ) ) return false;
		...
		switch ( $operation ) {
			...
			case 'manage_modules': return false !== strpos( $text, 'module' ) && $this->has_any( $text, array( 'manage','enable','disable','module' ) );
			case 'get_configuration': return false !== strpos( $text, 'config' ) && $this->has_any( $text, array( 'get','retrieve','configuration' ) ) && $this->jetengine_row( $row );
			...
			case 'import_configuration': return false !== strpos( $text, 'import' ) && $this->jetengine_row( $row );
			case 'export_configuration': return false !== strpos( $text, 'export' ) && $this->jetengine_row( $row );
		}
		return false;
	}
	private function resolve_operation( $operation ) { $matches = array(); foreach ( $this->native_rows() as $name => $row ) if ( $this->operation_matches( $operation, $row ) ) $matches[ $name ] = $row; if ( 1 !== count( $matches ) ) return new WP_Error( 0 === count( $matches ) ? 'mad4b_native_provider_operation_unavailable' : 'mad4b_native_provider_operation_ambiguous', 0 === count( $matches ) ? 'No unique provider-native ability matches the requested semantic operation.' : 'Multiple provider-native abilities match the requested semantic operation.', array( 'operation' => $operation, 'matches' => array_values( $matches ) ) ); $name = key( $matches ); return array( 'name' => $name, 'row' => current( $matches ), 'ability' => wp_get_ability( $name ) ); }

	public function provider_inventory( $input = array() ) { $hint = isset( $input['provider_hint'] ) ? $this->normalize_text( $input['provider_hint'] ) : ''; $write_only = ! empty( $input['write_only'] ); $items = array(); foreach ( $this->native_rows() as $row ) { $text = $this->row_text( $row ); if ( '' !== $hint && false === strpos( $text, $hint ) ) continue; if ( $write_only && true === $row['readonly'] ) continue; $items[] = $row; if ( count( $items ) >= 300 ) break; } return array( 'contract' => self::CONTRACT, 'count' => count( $items ), 'abilities' => $items ); }
	public function jetengine_inventory() { $ops = array( 'get_configuration','get_website_config','get_macros','create_cpt','create_taxonomy','create_meta_box','create_cct','create_query','create_glossary','create_listing','manage_modules','import_configuration','export_configuration' ); $items = array(); foreach ( $ops as $op ) { $resolved = $this->resolve_operation( $op ); $items[] = is_wp_error( $resolved ) ? array( 'operation' => $op, 'available' => false, 'error' => $resolved->get_error_code(), 'matches' => (array) $resolved->get_error_data() ) : array_merge( array( 'operation' => $op, 'available' => true ), $resolved['row'] ); } return array( 'contract' => self::CONTRACT, 'jetengine_available' => function_exists( 'jet_engine' ) || class_exists( 'Jet_Engine' ), 'operations' => $items, 'count' => count( $items ) ); }

	private function execute_resolved( $operation, array $input, $expect_write ) {
		$resolved = $this->resolve_operation( $operation ); if ( is_wp_error( $resolved ) ) return $resolved; $row = $resolved['row']; $expected_name = isset( $input['expected_native_ability'] ) ? (string) $input['expected_native_ability'] : ''; $expected_hash = isset( $input['expected_schema_sha256'] ) ? strtolower( (string) $input['expected_schema_sha256'] ) : '';
		...
		if ( $expect_write && true === $row['readonly'] ) return new WP_Error( 'mad4b_native_provider_readonly_mismatch', 'Resolved provider operation is read-only but a write operation was requested.' );
		$provider_input = isset( $input['input'] ) && is_array( $input['input'] ) ? $input['input'] : array(); return $resolved['ability']->execute( $provider_input );
	}
	...
	public function jetengine_export_configuration( $input ) { return $this->execute_resolved( 'export_configuration', $input, true ); }

	private function resolve_provider_semantic( $semantic, $provider_hint, $expected_name ) {
		$hint = $this->normalize_text( $provider_hint ); $matches = array(); foreach ( $this->native_rows() as $name => $row ) { $text = $this->row_text( $row ); if ( '' !== $hint && false === strpos( $text, $hint ) ) continue; if ( false === strpos( $text, $semantic ) ) continue; if ( '' !== $expected_name && $name !== $expected_name ) continue; $matches[ $name ] = $row; }
		if ( 1 !== count( $matches ) ) return new WP_Error( 0 === count( $matches ) ? 'mad4b_provider_semantic_operation_unavailable' : 'mad4b_provider_semantic_operation_ambiguous', 'Provider semantic operation did not resolve to exactly one native ability.', array( 'semantic' => $semantic, 'provider_hint' => $provider_hint, 'matches' => array_values( $matches ) ) ); $name = key( $matches ); return array( 'name' => $name, 'row' => current( $matches ), 'ability' => wp_get_ability( $name ) );
	}
	private function execute_provider_semantic( $semantic, array $input ) { $expected_name = (string) $input['expected_native_ability']; $resolved = $this->resolve_provider_semantic( $semantic, (string) $input['provider_hint'], $expected_name ); if ( is_wp_error( $resolved ) ) return $resolved; if ( ! hash_equals( $resolved['row']['schema_sha256'], strtolower( (string) $input['expected_schema_sha256'] ) ) ) return new WP_Error( 'mad4b_native_provider_schema_drift', 'Provider-native input schema changed since planning.', array( 'current_schema_sha256' => $resolved['row']['schema_sha256'] ) ); return $resolved['ability']->execute( isset( $input['input'] ) && is_array( $input['input'] ) ? $input['input'] : array() ); }
	public function provider_import_content( $input ) { return $this->execute_provider_semantic( 'import', $input ); }
	public function provider_export_content( $input ) { return $this->execute_provider_semantic( 'export', $input ); }
}
