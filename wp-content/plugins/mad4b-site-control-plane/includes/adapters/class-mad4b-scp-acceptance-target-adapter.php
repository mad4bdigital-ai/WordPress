...
/**
 * Registers the exact-candidate acceptance target adapter only on an enrolled Staging profile.
 * The concrete adapter class is declared lazily because this bootstrap is loaded before the
 * adapter base class. No Production or non-enrolled runtime receives this write surface.
 */
final class MAD4B_SCP_Acceptance_Target_Adapter_Bootstrap {
	...
	public static function register( $registry ) {
		...
		if ( ! class_exists( 'MAD4B_SCP_Acceptance_Target_Adapter', false ) ) {
			class MAD4B_SCP_Acceptance_Target_Adapter extends MAD4B_SCP_Adapter_Base {
				...
				public function provision( $input ) {
					...
					$record = wp_json_encode( $marker, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE );
					if ( false === $record ) return new WP_Error( 'mad4b_acceptance_target_marker_encode_failed', 'Acceptance target marker could not be encoded.' );
				...
				private function current_binding( array $expected = array(), $assert_expected = false ) {
					...
					foreach ( $required as $key ) if ( ! array_key_exists( $key, $expected ) ) return new WP_Error( 'mad4b_acceptance_target_binding_required', 'Exact revision, profile digest and build binding are required.' );
