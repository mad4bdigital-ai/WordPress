...
final class MAD4B_SCP_Servers {
	...
	public function register_servers( $adapter ) {
		...
		$this->create(
			...
			'mad4b-read',
			'MAD4B Read MCP',
			'Read-only discovery and diagnostics for WordPress, plugins, files and database.',
			$read_tools,
		...
		$this->create(
			...
			'mad4b-content',
			'MAD4B Content MCP',
			'Governed content editing abilities.',
			$content_tools,
			array( 'MAD4B_SCP_Policy', 'can_content' ),
			$transport,
			$error_handler,
			$observability
		);
		...
		$this->create(
			...
			'mad4b-admin',
			'MAD4B Admin MCP',
			'Administrative plugin, filesystem and structured database repair abilities.',
			$admin_tools,
		...
		$this->create(
			$adapter,
			'mad4b-breakglass',
			'MAD4B Breakglass MCP',
			'Exceptional recovery surface. Disabled unless explicitly enabled in wp-config.php.',
			array( 'mad4b/database-raw-query' ),
			array( 'MAD4B_SCP_Policy', 'can_breakglass' ),
			$transport,
			$error_handler,
			$observability
		);
