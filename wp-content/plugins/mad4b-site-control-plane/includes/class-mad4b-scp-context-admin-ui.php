...
final class MAD4B_SCP_Context_Admin_UI {
		...
		if ( self::is_ajax_request() ) {
			...
			wp_send_json_success(
				array(
					...
					'brand_core_coverage' => MAD4B_SCP_Context_Authority::brand_core_coverage(),
					'refresh' => true,
				)
			);
		}
	...
	private static function render_overview() {
		...
		if ( $pending_required_reviews + $content_changed_reviews > 0 && class_exists( 'MAD4B_SCP_Admin_Experience' ) ) {
			MAD4B_SCP_Admin_Experience::next_step(
				__( 'Human review required', 'mad4b-site-control-plane' ),
				sprintf( __( '%1$d required or changed Context asset(s) need an exact-content human decision before Brand Context can become ready.', 'mad4b-site-control-plane' ), $pending_required_reviews + $content_changed_reviews ),
				'attention',
				self::tab_url( 'assets', array( 'review_filter' => 'needs_review' ) ),
				__( 'Review required assets', 'mad4b-site-control-plane' )
			);
		}
	...
	private static function render_assets() {
		...
		echo '<div class="mad4b-scp-panel mad4b-context-assets-panel"><h2>' . esc_html__( 'Context Assets', 'mad4b-site-control-plane' ) . '</h2>';
	...
	private static function scripts() {
		...
		(function(){
			...
			async function refreshReviewPanels(){
				var response=await fetch(window.location.href,{credentials:"same-origin",headers:{"Cache-Control":"no-cache"}});
			...
			document.addEventListener("submit",async function(event){
				...
				try{
					...
					if(!payload||!payload.success){
						var failure=new Error(payload&&payload.data&&payload.data.message?payload.data.message:"Context review could not be committed.");
						failure.mad4bCode=payload&&payload.data&&payload.data.code?payload.data.code:"mad4b_context_review_failed";
						throw failure;
					}
