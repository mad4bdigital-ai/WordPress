...
final class FilterContextBuilder {
    ...
    public function build( ?string $uri = null ): array { return $this->buildInternal( $uri, false, null ); }
    public function buildEvidence( ?string $uri = null, ?string $language = null ): array { return $this->buildInternal( $uri, true, $language ); }
    public function buildAjaxEvidence( array $payload, ?string $language = null ): array {
        ...
        return $this->buildParsed( $parsed, true, $language, $uri );
    }
    private function buildInternal( ?string $uri, bool $evidenceOnly, ?string $languageOverride ): array {
        $parsed=$this->parser->parse($uri);
        return $this->buildParsed( $parsed, $evidenceOnly, $languageOverride, $uri );
    }
    private function buildParsed( array $parsed, bool $evidenceOnly, ?string $languageOverride, ?string $languageUri ): array {
        ...
        $runtime=$ajax?array('observed'=>true,'provider'=>(string)($parsed['provider']??''),'query_id'=>(string)($parsed['query_id']??''),'source'=>'jet_smart_filters_browser_event'):$this->currentProvider();
        ...
        $providerMatchState=$ajax&&!empty($runtime['observed'])&&$runtime['provider']===(string)($parsed['provider']??'')&&$runtime['query_id']===(string)($parsed['query_id']??'');
        ...
        $context=array_merge($parsed,array(
            ...
            'jet_smart_filters_provider'=>$runtime,'provider_observed'=>!empty($runtime['observed']),'provider_observation_matches_url'=>$providerMatchUrl,'provider_observation_matches_state'=>$providerMatchState,
            ...
            'result_count'=>null,'result_count_source'=>'unavailable','result_count_authoritative'=>false,'result_count_detail'=>'','combination_authority'=>array(),'content_readiness'=>array(),'evidence_only'=>$evidenceOnly,'synthetic'=>false,'authorizing'=>!$evidenceOnly&&!empty($scope['authorizing']),'global_enabled'=>!empty($scope['global_enabled']),
