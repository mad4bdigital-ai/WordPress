...
final class InventoryContentCatalog {
    const CONTRACT='etg.dfsb.inventory-content-catalog.v3';
    ...
    public function build(array $snapshot,array $profiles=array()):array{
        ...
        foreach((array)$profiles as $profileKey=>$profile){
            ...
            foreach((array)($profile['routes']??array()) as $route){
                ...
                $queryId=QueryId::normalize($route['provider_query_id']??($route['query_id']??''));
        ...
        foreach($roleTaxonomies as $role=>$taxonomyMap){
            ...
            foreach(array('ids'=>'Selected IDs','names'=>'Selected Names','slugs'=>'Selected Slugs','count'=>'Selected Count','descriptions'=>'Selected Descriptions','short_descriptions'=>'Selected Short Descriptions') as $field=>$label){$type=in_array($field,array('descriptions','short_descriptions'),true)?'html':'text';$this->token($tokens,'terms:'.$role.':'.$field,ucwords(str_replace('_',' ',$role)).' — '.$label,$type,'ajax-term-set');}
            foreach(array_keys($taxonomyMap) as $taxonomy){foreach($this->metaKeys($taxonomy) as $key){$this->token($tokens,'termmeta:'.$role.':'.$key,ucwords(str_replace('_',' ',$role)).' Meta — '.$key,'text','term-meta',array('field_key'=>$key));}}
        }
        $topology=(array)($inventory['elementor_topology']??array());
        foreach(array_slice((array)($topology['bindings']??array()),0,100) as $binding){
            if(!is_array($binding)||'verified'!==(string)($binding['status']??'')){continue;}
            $providerId=QueryId::normalize($binding['provider_query_id']??'');
            $qb=QueryId::normalize($binding['query_builder_custom_query_id']??'');
            $tokenProviderId=QueryId::tokenKey($providerId);
            if($providerId&&$tokenProviderId){
                $this->token($tokens,'topology:'.$tokenProviderId.':query_builder_query_id','Topology '.$providerId.' Query Builder ID','text','topology',array('provider_query_id'=>$providerId,'query_builder_query_id'=>$qb,'internal_id'=>(string)($binding['query_builder_internal_id']??''),'post_types'=>(array)($binding['post_types']??array())));
            }
        }
