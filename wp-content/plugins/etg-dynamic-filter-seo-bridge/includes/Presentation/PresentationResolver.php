...
final class PresentationResolver {
    ...
    private function resolvedSourceValue(array $slot,array $c):string{
        ...
        if(!$this->sources){if('image'===(string)($slot['type']??'')){return(string)($this->image($mediaMode,$c)['id']??0);}if('gallery'===(string)($slot['type']??'')){return implode(',',$this->gallery->ids($c,$mediaMode));}return'';}
