...
final class PresentationResolver {
    ...
    public function value(string $token,array $context=null){
        ...
        if('gallery_ids'===$token){$media=$this->mediaArray($this->gallery->ids($c,'combined'));return implode(',',array_map(function($item){return(int)($item['id']??0);},$media));}
    ...
    public function mediaTrace(string $mode='combined',array $context=null,int $limit=30):array{
        $c=null===$context?$this->context():$context;if(!$this->renderable($c)){return array('contract'=>'etg.dfsb.media-trace.v1','authorizing'=>false,'mode'=>sanitize_key($mode),'count'=>0,'items'=>array());}
        return$this->gallery->trace($c,$mode,$limit);
    }

    public function mediaModeOptions(array $context=null):array{$c=null===$context?$this->context():$context;return$this->gallery->modeOptions(is_array($c)?$c:array());}
    ...
    public function slotMediaMode(string $id):string{$slot=$this->slots->get($id);return(string)($slot['media_mode']??('image'===(string)($slot['type']??'')?'priority':'combined'));}
    ...
    private function resolvedSourceValue(array $slot,array $c):string{
        $mediaMode=sanitize_key((string)($slot['media_mode']??('image'===(string)($slot['type']??'')?'priority':'combined')));
        if(!$this->sources){if('image'===(string)($slot['type']??'')){return(string)($this->image($mediaMode,$c)['id']??0);}if('gallery'===(string)($slot['type']??'')){$media=$this->gallery($mediaMode,$c,30);return implode(',',array_map(function($item){return(int)($item['id']??0);},$media));}return'';}
        ...
        if('image'===(string)($slot['type']??'')){return(string)($this->image($mediaMode,$c)['id']??0);}if('gallery'===(string)($slot['type']??'')){$media=$this->gallery($mediaMode,$c,30);return implode(',',array_map(function($item){return(int)($item['id']??0);},$media));}return'';
    }
    ...
    private function termValue(array $c,string $role,string $field){$term=(array)($c['terms'][$role]??array());if(!$term){return'';}if(in_array($field,array('image_id','image_url'),true)){$id=(int)($term['image_id']??0);if(!$id){return'image_id'===$field?0:'';}$media=$this->mediaArray(array($id));if(!$media){return'image_id'===$field?0:'';}return'image_id'===$field?(int)($media[0]['id']??0):(string)($media[0]['url']??'');}if(!array_key_exists($field,$term)){return'';}$v=$term[$field];return is_scalar($v)?$v:'';}
