...
final class MetadataAdapter {
	...
	public function robots( $robots ) {
		...
		if ( ! array_key_exists('index',$decision) || null === $decision['index'] || ! is_array( $robots ) ) { return $robots; }
		$robots['index'] = $decision['index'] ? 'index' : 'noindex';
		$robots['follow'] = 'follow';
		return $robots;
	}

	public function openGraphType( $original ) {
		$context=$this->context();
		return $this->socialAllowed($context) ? 'website' : $original;
	}

	public function openGraphUrl( $original ) {
		$context=$this->context();
		if(!$this->socialAllowed($context)){return $original;}
		$url=$this->canonical->build($context,(string)$original);
		return $url?:$original;
	}

	public function socialTitle( $original ) {
		$context=$this->context();
		if(!$this->socialAllowed($context)){return $original;}
		$title=$this->content->metaTitle($context);
		return ''!==$title?$title:$original;
	...
	public function socialDescription( $original ) {
		$context=$this->context();
		if(!$this->socialAllowed($context)){return $original;}
		$description=$this->content->metaDescription($context);
		return ''!==$description?$description:$original;
	...
	public function socialImage( $original ) {
		$context = $this->context();
		if ( ! $this->socialAllowed( $context ) ) { return $original; }
	...
	public function twitterCardType( $original ) {
		$context=$this->context();
		if(!$this->socialAllowed($context)){return $original;}
		$ids=$this->gallery->ids($context,'priority');
		return $ids?'summary_large_image':$original;
	}

	public function schema( $data, $jsonld = null ) {
		if(!is_array($data)||!$this->publication){return $data;}
		$context=$this->context();
		if(!$this->hasResolvedContext($context)){return $data;}
		$decision=$this->indexing->decide($context);
		if(true!==($decision['index']??null)){return $data;}
		$schema=$this->publication->schemaForContext($context);
		if($schema){$data['ETGFilteredCollection']=$schema;}
		return $data;
	}

	public function hreflangs( $items ) {
		if(!$this->publication){return $items;}
		$context=$this->context();
		if(!$this->hasResolvedContext($context)){return $items;}
	...
	private function socialAllowed( array $context ): bool {
		if(!$this->hasResolvedContext($context)){return false;}
		$profile=(array)($context['profile']??array());
		return !isset($profile['publication']['social'])||!empty($profile['publication']['social']);
	}
