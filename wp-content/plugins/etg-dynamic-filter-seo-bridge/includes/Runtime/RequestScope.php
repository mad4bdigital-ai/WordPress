...
final class RequestScope {
	...
	public function evaluateForEvidence( array $parsed ): array {
		...
		else { $scope=$this->profiles->resolveForEvidence( $parsed ); }
		$scope['authorizing']=false;$scope['evidence_only']=true;$scope['global_enabled']=$this->config->enabled();return $scope;
	}
	private function decision( bool $inScope, bool $valid, string $reason ): array {
		return array('in_scope'=>$inScope,'scope_valid'=>$valid,'reason'=>$reason,'profile_id'=>'','profile'=>array(),'configuration_revision'=>$this->config->revision(),'authorizing'=>false,'evidence_only'=>false,'global_enabled'=>$this->config->enabled());
	}
}
