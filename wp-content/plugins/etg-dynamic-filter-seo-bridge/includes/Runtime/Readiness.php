...
final class Readiness {
	...
	public function __construct( Compatibility $compatibility, Configuration $config, ProfileRegistry $profiles ) { $this->compatibility=$compatibility; $this->config=$config; $this->profiles=$profiles; }
	...
	public function report(): array {
		...
		if ( ! $this->config->enabled() ) { $status = 'inactive'; }
		elseif ( $missing || $missingCapabilities || $configErrors || $pending || $failedRuntime ) { $status = 'degraded'; }
		else { $status = 'ready'; }
		return array(
			'contract' => 'etg.dfsb.readiness.v3',
			'status' => $status,
	...
	public function ready(): bool { $report = $this->report(); return 'ready' === $report['status']; }
}
