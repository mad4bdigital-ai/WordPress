...
final class Configuration {
	...
	public function all(): array {
		...
		if ( function_exists( 'apply_filters' ) ) {
			$config = (array) apply_filters( 'etg_filter_seo_configuration', $config );
			$config = $this->sanitize( $config );
		}
		return $config;
	}
		...
		$profiles = array(
			array(
				...
				'content' => array(
					...
					'require_meta_description'=>true,
					'min_chars'=>250,
					'min_chars_by_depth'=>array( '1'=>250, '2'=>400, '3'=>500 ),
					'min_unique_segments_by_depth'=>array( '1'=>1, '2'=>2, '3'=>2 ),
				),
				'publication' => array(
					'sitemap'=>true,
					'hreflang'=>true,
	...
	private function profilesJson( $value ): string {
		...
		if ( is_array( $value ) ) { $decoded = $value; }
			...
			if ( '' === $value || strlen( $value ) > 1000000 ) { $valid=false; $decoded=array(); }
				...
				if ( JSON_ERROR_NONE !== json_last_error() || ! is_array( $decoded ) ) { $valid=false; $decoded=array(); }
			}
		}
		if ( ! $valid ) {
		...
		if ( count( $decoded ) > 50 ) {
			if(function_exists('add_settings_error')){add_settings_error('etg_dfsb','profiles_limit','Surface Profiles JSON exceeds the 50-profile Alpha limit; the previous valid snapshot was preserved.','error');}
			return $this->previousProfilesJson();
		}
	...
	private function lineList( $value ): array {
		if ( is_string( $value ) ) { $value = preg_split( '/[\r\n]+/', $value ); }
		$value = is_array( $value ) ? $value : array();
		$out=array();
		foreach($value as $line){$line=strtolower(trim((string)$line)); if(''!==$line){$out[]=$line;}}
		return array_values(array_unique($out));
	}
	private function normalizeList( $value, string $sanitizer ): array {
		if ( is_string( $value ) ) { $value = preg_split( '/[\r\n,]+/', $value ); }
		$value=is_array($value)?$value:array();
		$out=array();
		foreach($value as $item){$item=call_user_func($sanitizer,(string)$item); if(''!==$item){$out[]=$item;}}
		return array_values(array_unique($out));
	}
