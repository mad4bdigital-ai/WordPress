...
trait InventoryReconcilerSupportTrait {
    private function validateSnapshot(array $snapshot):array{
        ...
        $seen=array();foreach($conflicts as $conflict){if(!is_array($conflict)){$errors[]='inventory_query_identity_conflict_record_invalid';continue;}$key=sanitize_key((string)($conflict['identity_key']??''));if(''===$key||isset($seen[$key])||(int)($conflict['count']??0)<2){$errors[]='inventory_query_identity_conflict_record_invalid';continue;}$seen[$key]=true;}
    ...
    private function queryConflictIndex(array $conflicts):array{$out=array();foreach(array_slice($conflicts,0,RuntimeInventory::MAX_QUERY_IDENTITY_CONFLICTS) as $conflict){if(!is_array($conflict)){continue;}$key=sanitize_key((string)($conflict['identity_key']??''));if(''!==$key){$out[$key]=$conflict;}}ksort($out,SORT_STRING);return $out;}
