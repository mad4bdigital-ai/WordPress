...
trait InventoryReconcilerBindingTrait {
    private function resolveRouteIdentity(string $provider,string $providerQueryId,array $route,array $queryIndex,array $conflicts,array $topology):array{
        $explicit=QueryId::normalize($route['query_builder_query_id']??'');
        ...
        foreach((array)($topology['bindings']??array()) as $binding){if(!is_array($binding)){continue;}if($provider!==sanitize_key((string)($binding['provider']??''))||$providerQueryId!==QueryId::normalize($binding['provider_query_id']??'')){continue;}if('verified'===(string)($binding['status']??'')){$qid=QueryId::normalize($binding['query_builder_custom_query_id']??'');if(''!==$qid){$matches[$qid][]=$binding;}}else{$blocked[]=$binding;}}
