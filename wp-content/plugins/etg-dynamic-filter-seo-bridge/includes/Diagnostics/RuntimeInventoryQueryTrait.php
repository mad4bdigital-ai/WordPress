...
trait RuntimeInventoryQueryTrait {
	private function queries(): array {
		...
		foreach ( is_array( $raw ) ? $raw : array() as $query ) {
			...
			$structuralKey = hash( 'sha256', $this->encode( array(
		...
		usort( $records, static function ( $a, $b ) {
			foreach ( array( 'identity_key', 'id', 'type', 'structural_key' ) as $field ) {
		...
		foreach ( $records as $record ) {
			...
			$identityMap[ $key ][] = array(
				'id' => (string) $record['id'],
				'custom_query_id' => (string) $record['custom_query_id'],
				'type' => (string) $record['type'],
				'post_types' => (array) $record['post_types'],
				'post_type_bounded' => ! empty( $record['post_type_bounded'] ),
				'taxonomies' => (array) $record['taxonomies'],
			);
		}
