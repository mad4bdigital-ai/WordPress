...
final class GalleryComposer {
    public function ids( array $context, string $mode = 'combined' ): array {
        ...
        // Role-specific media is useful for Elementor Image/Gallery controls.
        if ( ! in_array( $mode, array( 'combined', 'priority' ), true ) && isset( $terms[ $mode ] ) ) {
            return array_values( array_unique( array_filter( array_map( 'absint', $this->termIds( (array) $terms[ $mode ] ) ) ) ) );
        }

        $roles = $this->orderedRoles( $context, 'priority' === $mode );
        if ( 'priority' === $mode ) {
            foreach ( $roles as $role ) {
                $ids = $this->termIds( isset( $terms[ $role ] ) ? (array) $terms[ $role ] : array() );
                if ( $ids ) { return array_values( array_unique( array_filter( array_map( 'absint', $ids ) ) ) ); }
            }
            return array();
        }
        ...
        $ids = array();
        foreach ( $roles as $role ) {
            $ids = array_merge( $ids, $this->termIds( isset( $terms[ $role ] ) ? (array) $terms[ $role ] : array() ) );
        }
