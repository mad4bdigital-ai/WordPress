...
final class PublicationCache {
	...
	private function key(string $identity):string{return self::KEY_PREFIX.substr(hash('sha256',$this->epoch().'|'.$this->config->revision().'|'.$identity),0,32);}private function epoch():int{if(!function_exists('get_option')){return 1;}$value=get_option(self::EPOCH_OPTION,1);return is_numeric($value)?max(1,(int)$value):1;}
}
