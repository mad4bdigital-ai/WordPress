...
- Added `JetEngine\QueryIdentityResolver`, which resolves exact custom Query IDs only from `Manager::get_queries()` and returns the internal numeric Query Builder ID as evidence, never as fallback authority.
- Missing custom IDs now fail closed as `query_identity_not_found`; duplicate custom IDs fail closed as `query_identity_ambiguous`; unavailable inventory fails closed as `query_identity_inventory_unavailable`.
- Numeric-looking custom Query IDs are kept in the custom-ID namespace and cannot accidentally resolve through the internal Query Builder ID namespace.
- Injected one shared resolver into Post Type observation, request-time authoritative result-count resolution, and background publication result-count probing so all three surfaces use identical identity semantics.
- Post Type and result-count evidence now exposes both `custom_query_id` and `internal_query_id` without changing profile/provider authority, which remains bound to the custom route Query ID.
- Added PHP smoke coverage for unique, missing, duplicate, numeric-looking and invalid-inventory identity cases plus static guards preventing the three route-identity consumers from reintroducing `get_query_by_id()`.
- Global bridge remains OFF by default, the built-in Tours profile remains disabled, and this release does not authorize merge or Production activation.
...
- Live ETG sitemap publication is empty while Global is OFF and only includes URLs whose final `IndexingPolicy` decision is `index=true`.
