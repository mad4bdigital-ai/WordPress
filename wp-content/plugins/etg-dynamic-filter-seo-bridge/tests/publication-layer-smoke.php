...
$assert(false !== strpos($files['shortcodes'], "'disabled'!==(string)"), 'dark presentation disabled-state guard missing');
...
foreach (array('etg_filter_term', 'etg_filter_term_section', 'etg_filter_sections') as $shortcode) {
    $assert(false !== strpos($files['shortcodes'], $shortcode), 'missing Elementor shortcode:' . $shortcode);
}
...
foreach (array('rank_math_sitemap_provider_interface', 'rank_math_sitemap_router', 'rank_math_sitemap_cache', 'wpml_active_languages_filter', 'wpml_permalink_filter') as $cap) {
    $assert(false !== strpos($files['compat'], $cap), 'missing publication capability:' . $cap);
}
...
/* Behavioral proof: dark presentation may render resolved Term content only
 * when the normal request is stopped by Global OFF and the profile opts in. */
...
$normal = array(
    ...
    'scope_valid' => false,
    'runtime_ready' => false,
    'filters' => array('location_jet' => 'cairo'),
    'scope' => array('reason' => 'disabled'),
);
$evidence = array(
    'active' => true,
    'in_scope' => true,
    ...
    'provider_observation_matches_url' => true,
    'profile' => array(
        'publication' => array('elementor_render_when_global_off' => true),
        'taxonomy_rules' => array('location_jet' => array('role' => 'location', 'priority' => 10)),
    ),
    ...
    'taxonomy_roles' => array('location_jet' => 'location'),
    'language' => 'en',
);
