...
$slots=new ContentSlotRegistry(true);$defaults=$slots->all();foreach(array('hero_title','hero_intro','location_section','tour_type_section','style_section','results_summary') as $slotId){expect_true(isset($defaults[$slotId]),'built-in slot exists: '.$slotId);}expect_same(array(),$GLOBALS['etg_options'],'built-in slots do not mutate WordPress options');
