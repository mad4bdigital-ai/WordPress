...
$manager->queries = array(
    new ETG_Test_Query( 77, 'tours_query_archive' ),
    new ETG_Test_Query( 78, 'myGrid' ),
    new ETG_Test_Query( 79, 'mygrid' ),
);
...
$context = array( 'provider'=>'jet-engine','query_id'=>'myGrid','provider_observation_matches_url'=>true );
$result = $adapter->resolve( $context );
expect_same( 4, $result['count'], 'mixed-case Query ID still resolves filtered total' );
expect_same( 'myGrid', $result['provider_query_id'], 'provider Query ID preserves mixed case' );
expect_same( 'myGrid', $result['custom_query_id'], 'Query Builder custom ID preserves mixed case' );
expect_same( '78', $result['internal_query_id'], 'mixed-case identity resolves the exact Query Builder record' );
...
require __DIR__ . '/alpha13-query-id-boundary-smoke.php';
