...
etg_result_count_expect(false !== strpos($observer, 'function domIds(limit)'), 'browser observer uses an explicit bounded DOM ID collector');
etg_result_count_expect(false !== strpos($observer, 'Math.min(5000'), 'full digest proof collection remains bounded to 5000 IDs');
etg_result_count_expect(false !== strpos($observer, 'proofIds.slice(0, 100)'), 'transported rendered IDs remain bounded to 100 even when full digest proof is larger');
