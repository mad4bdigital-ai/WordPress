...
$GLOBALS['etg_options']=array();$GLOBALS['etg_force_soft_index']=null;
...
function get_bloginfo($key){return 'ETG';}
...
function apply_filters($hook,$value){if('etg_filter_seo_should_index'===$hook&&null!==$GLOBALS['etg_force_soft_index']){return(bool)$GLOBALS['etg_force_soft_index'];}if('etg_filter_seo_content_ready'===$hook&&null!==$GLOBALS['etg_content_filter']){return(bool)$GLOBALS['etg_content_filter'];}return $value;}
...
expect_same(false,$config->enabled(),'safe default is disabled');
...
$GLOBALS['etg_options'][Configuration::OPTION_NAME]=array('enabled'=>true,'indexable_combinations'=>array('en|location_jet=cairo|tour-types_jet=day-tours'));
...
expect_same(true,$scopeDecision['in_scope'],'configured surface in scope');
...
expect_same(false,in_array('tour-types_jet',$singleProfile['allowed_taxonomy_sets'],true),'legacy single tour-type opt-in cannot synthesize structural single-taxonomy authority');
$explicitSingleProfile=$singleProfile;
$explicitSingleProfile['inherit_global_defaults']=false;
$explicitSingleProfile['allowed_taxonomy_sets'][]='tour-types_jet';
$GLOBALS['etg_options'][Configuration::OPTION_NAME]['profiles_json']=json_encode(array($explicitSingleProfile));
$explicitSingleConfig=new Configuration();$explicitSingleRegistry=new ProfileRegistry($explicitSingleConfig);$explicitSingleProfile=$explicitSingleRegistry->get('tours');
expect_same(true,in_array('tour-types_jet',$explicitSingleProfile['allowed_taxonomy_sets'],true),'explicit profile-local taxonomy set grants structural single-taxonomy authority');
$GLOBALS['etg_options'][Configuration::OPTION_NAME]['profiles_json']=json_encode(array($profile));
...
expect_same('Cameras — Sony',$content->title($generic),'generic title follows configured taxonomy priority');
$explicit=array('language'=>'en','profile'=>array('composition_mode'=>'generic'),'terms'=>array('topic'=>array('name'=>'SEO','seo_title'=>'SEO Knowledge Base')));
expect_same('SEO Knowledge Base | ETG',$content->metaTitle($explicit),'generic term seo_title participates in metadata');
...
expect_same('https://example.com/it/tours-and-activities/',$canonical,'profile canonical mode preserves language-aware archive path');

$policy=new IndexingPolicy($config);$sim=new ScenarioSimulator($config,$registry,$combos,$policy);
$baseScenario=array('profile_id'=>'tours','language'=>'en','filters'=>array('location_jet'=>'cairo','tour-types_jet'=>'day-tours'),'result_count'=>3,'result_count_authoritative'=>true,'runtime_ready'=>true,'provider_match'=>true,'content_ready'=>true);
$decision=$sim->run($baseScenario)['decision'];expect_same(true,$decision['index'],'approved ready tours pair at threshold indexes');
$GLOBALS['etg_force_soft_index']=true;$low=$baseScenario;$low['result_count']=2;$decision=$sim->run($low)['decision'];expect_same(true,$decision['index'],'soft threshold can be intentionally overridden');expect_same(true,$decision['override_applied'],'soft override is explicit');
...
expect_same(false,$deduped['ready'],'duplicate term copy is counted once for unique-content readiness');
...
expect_same(false,$promoteAttempt['ready'],'content readiness filter cannot promote a hard content failure');
expect_same('attempted_promote_blocked',$promoteAttempt['override_direction'],'blocked promotion is explicit in evidence');
...
expect_same(false,$veto['ready'],'content readiness filter may veto an otherwise ready page');
